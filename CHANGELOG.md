...
### Boot / tela preta
- Corrige a causa real da tela preta herdada da r295: `runtime-r295-browse-actions-self-scope-fix.js` era executado antes de `boot()` e tratava a ausência temporária de `paintBrowse263` como erro fatal.
- O self-scope r295 passa a iniciar em estado pendente e instalar sua decoração somente quando `__ctR295Test.decorateBrowseActions` e `paintBrowse263` estiverem disponíveis, com reconciliação finita após o boot.
- Remove a exceção fatal `r295 browse self-scope fix missing r295 authority`; a proteção continua existindo, mas não derruba mais o bundle durante a fase legítima em que a autoridade de pintura ainda não foi criada.

### Build / validação
- Web atualizada para `1.0.88 / r297-official-1.0.88`; Android permanece `1.0.20 / versionCode 10062` sem alteração.
- Adiciona regressão Chromium sobre o bundle final completo `app-v297.js`, exigindo passagem real por `boot()`, `#app` renderizado e self-scope r295 efetivamente instalado depois que a autoridade fica disponível.
- O `production_smoke` passa a validar a identidade r297, a ausência da exceção fatal pré-boot e a renderização pública antes de considerar a release concluída.
