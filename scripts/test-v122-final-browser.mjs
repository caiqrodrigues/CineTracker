...
const runtimeFiles=[
  'apps/web/runtime-r228-v122-web-authority.js',
  'apps/web/runtime-r228b-v122-card-metadata.js',
  'apps/web/runtime-r228c-v122-authority-guards.js',
...
<div id="app">
  ...
  <div data-profile>
    <section><div class="panel-head"><h2>Estatísticas</h2><button id="collapse">Recolher</button></div></section>
    <button class="stat" data-ct120-watchlist="series"><small>Séries Watchlist</small><b>564</b></button>
    <button class="stat" data-ct120-watchlist="movie"><small>Filmes Watchlist</small><b>1.358</b></button>
  </div>
</div>
<script>
...
(async()=>{try{
  ...
  document.body.dataset.movieCount=document.querySelector('[data-ct122-watchlist="movie"] b')?.textContent||'';
  document.body.dataset.seriesCount=document.querySelector('[data-ct122-watchlist="series"] b')?.textContent||'';
  document.body.dataset.rowsMovie=String(window.__ctV122Rows(${JSON.stringify(payload)},'movie').length);
  document.body.dataset.rowsSeries=String(window.__ctV122Rows(${JSON.stringify(payload)},'series').length);
  document.body.dataset.actions=String(sport.querySelectorAll('.ct122-action').length);
...
console.log('V122_FINAL_BROWSER_OK discover=semantic+css-loading metadata=year+genres sports=exact-two-actions counts=exact-renderable stats=icon-only');
