...
const preboot = resolve(web, 'auth-preboot-fix7.js');
...
let safe = raw.replace('void bootstrap();', 'window.__ctBaseBootstrapDeferred = bootstrap;');
const marker = '<div id="app"></div><script>';
if (!safe.includes(marker)) throw new Error('App/base script marker not found; FIX 7 preboot cannot be installed.');
safe = safe.replace(marker, '<div id="app"></div><script src="/auth-preboot-fix7.js"></script><script>');
const withIcon = safe.includes('rel="icon"') ? safe : safe.replace('</head>', '<link rel="icon" type="image/svg+xml" href="/favicon.svg"></head>');
...
if (!built.includes('/auth-preboot-fix7.js')) throw new Error('FIX 7 preboot missing from built HTML.');
if (!built.includes('patch-v073-v097-fix7.js')) throw new Error('FIX 7 auth owner missing from built HTML.');
if (built.indexOf('/auth-preboot-fix7.js') > built.indexOf('const SUPABASE_URL')) throw new Error('FIX 7 preboot must run before base auth definitions.');
...
for (const dist of [rootDist, webDist]) {
  ...
  await cp(preboot, resolve(dist, 'auth-preboot-fix7.js'));
  for (const f of patches) await cp(f, resolve(dist, f.split('/').pop()));
}
console.log('CineTracker Web v0.0.97 FIX 7: preboot quarantine + deferred base bootstrap + single auth owner');
