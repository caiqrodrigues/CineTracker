...
['FIX timeout auth',fix.includes('AbortController')&&fix.includes('AUTH_TIMEOUT_MS=12000')&&fix.includes('TimeoutError')],
['FIX loading finalizado',fix.includes('finally{setLoginLoading(false)}')&&fix.includes("form.dataset.loading==='1'")],
['FIX sessão e Home',fix.includes('saveSession(d)')&&fix.includes('persistNativeSession')&&fix.includes("view='home'")&&fix.includes('goHomeFix')],
['FIX watchdog restore',fix.includes('restoreSavedSessionFix')&&fix.includes('recoverStuckAuth')],
