...
const p91=src['patch-v061-v091.js'],p92=src['patch-v063-v092.js'],v98=src['patch-v089-v098.js'],pre=src['patch-v088-v098-nav-pre.js'],v99=src['patch-v091-v099-profile-lru.js'],v991=src['patch-v092-v0991.js'],v992=src['patch-v093-v0992.js'],v992c=src['patch-v094-v0992-compat.js'],fix=src['patch-v095-v0992-fix.js'],unfreeze=src['patch-v096-v0992-unfreeze.js'],sem=src['patch-v083-hotfix13-bingers-semantics.js'],h16=src['patch-v087-hotfix16-import-resilience.js'],sw=src['service-worker.js'];
const namesStart=selective.indexOf('const names=['),orderTokens=['pre98',"'patch-v085-hotfix15-import-transport.js'","'patch-v075-hotfix10-selective.js'","'patch-v082-hotfix12-picker-guard.js'","'patch-v083-hotfix13-bingers-semantics.js'","'patch-v087-hotfix16-import-resilience.js'",'profileName','ui98','compat98','profile99','release991','release992','compat992','fix992','unfreeze992'];let cursor=namesStart,ordered=namesStart>=0;for(const token of orderTokens){const next=selective.indexOf(token,cursor+1);if(next<0){ordered=false;break}cursor=next}
const checks=[
['package Web 0.99.2',pkg.includes('"version": "0.99.2"')],
['cache Web 0.99.2 FIX2',sw.includes("ct-web-0.99.2-fix2")&&!sw.includes('index.html')],
['Android 0.99.2 / code 9913',gradle.includes('versionCode 9913')&&gradle.includes("versionName '0.99.2'")],
['Histórico continua fora da navegação Android',layout.includes('nav_history')&&layout.includes('android:visibility="gone"')],
['pre-nav conhece Home 0.99.2',pre.includes('ct0992Navigate')&&pre.includes("target === 'home'")],
['unfreeze final carregado por último',ordered&&selective.includes("const unfreeze992='patch-v096-v0992-unfreeze.js'")],
...
['navegação autoritativa Window capture',fix.includes("window.addEventListener('click'")&&fix.includes('e.stopImmediatePropagation()')&&fix.includes('window.ct0992Navigate=navigate;window.ct991Navigate=navigate;window.ct98Navigate=navigate')],
['nav final somente Home Descobrir Perfil Configurações',fix.includes("['home','⌂ Home','Home']")&&fix.includes("['discover','✦ Descobrir','Descobrir']")&&fix.includes("['profile','◉ Perfil','Perfil']")&&fix.includes("['settings','⚙ Configurações','Config.']")&&!fix.includes("['history'" )],
...
['Surrogate negativo abre detalhe local',v992c.includes('async function openLocal')&&v992c.includes('Mídia importada sem TMDB oficial')&&fix.includes('function openLocal(row)')],
...
['FIX2 elimina churn recursivo de textContent',unfreeze.includes('fix2-idempotent-dom-mutation-guard')&&unfreeze.includes("Object.getOwnPropertyDescriptor(nodeProto, 'textContent')")&&unfreeze.includes('desc.get.call(this) === next')&&unfreeze.includes('__ctTextContentIdempotent992')],
...
['Perfil 0.99.1 preservado',v991.includes('profileBusy991')&&v991.includes('Consumo temporal')&&v991.includes('Tempo p/ séries em dia')&&v99.includes('last_watched_at')],
...
['Rodapé 0.99.2 autoritativo',fix.includes('CineTracker • v0.99.2')&&fix.includes('window.__ctAndroidBuild=VERSION')],
['Android FIX2 bundle 0.99.2',prepare.includes('v0.99.2-fix2-unfreeze-991-992-authoritative')&&prepare.includes('patch-v096-v0992-unfreeze.js')&&prepare.includes('__ct0992UnfreezeLoaded')],
