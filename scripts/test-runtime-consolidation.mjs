...
const forbiddenPatches=[
  'patch-v025.js','patch-v027.js','patch-v029.js','patch-v030.js','patch-v034.js',
...
for(const rel of ['dist','apps/web/dist']){
  ...
  for(const marker of ['__ct0997AuthoritativeLoaded','__ct0997RealSmoke119Loaded','__ct0997Structural120Loaded','cinetracker_profile_payload_v0997','cinetracker_discovery_exclusions_v0994','ct-enrich-media-user','__ctRuntimeCoreDataLoaded','0.99.7-rest-cache-v1']){
  ...
  for(const legacy of ['ct30-open-history','ct44-hour-chart','setInterval(scan,3000)','Histórico diário</h3><div class="ct95-daily" id="ct17-daily"','row=rows.find(x=>String(x.title||x.name||\'\').toLowerCase()===title.toLowerCase())||rows[0]']){
...
console.log('Runtime consolidation parity: PASS — one emitted runtime, superseded UI authorities absent, canonical data core + 0.99.7 behavior retained.');
