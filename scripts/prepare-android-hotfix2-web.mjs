...
const hotfixMarker = `<script>\nwindow.__ctAndroidBundle = 'hotfix17-series-status-v95-core-inline-authoritative';\nwindow.__ctHotfix10Startup = true;\nwindow.__ctAndroidBuild = '0.0.97 HOTFIX 17';\n</script>`;
html = html.replace('</body>', hotfixMarker + '</body>');
html = html.replace(/<link rel="icon"[^>]*>/g, '');
await writeFile(indexPath, html, 'utf8');

if (!html.includes("window.__ctAuthRecovery = 'v97-hotfix6-startup'")) throw new Error('Android HOTFIX17 missing startup auth recovery.');
if (!html.includes("window.__ctP0SessionReset = 'hotfix7-once'")) throw new Error('Android HOTFIX17 missing one-time poisoned-session reset.');
if (!html.includes('data-ct-inline="patch-v067-v095.js"')) throw new Error('Android HOTFIX17 missing stable v95 feature layer.');
...
for (const name of required) if (!html.includes(`data-ct-inline="${name}"`)) throw new Error(`Android HOTFIX17 missing ${name}.`);
if (html.includes('data-ct-inline="patch-v081-hotfix12-nav-pre.js"')) throw new Error('Android HOTFIX17 contains obsolete HOTFIX12 capture nav.');
if (html.includes('data-ct-inline="patch-v084-hotfix14-real-device.js"')) throw new Error('Android HOTFIX17 contains obsolete HOTFIX14 import layer.');
...
if (navIndex < 0 || selectiveIndex < 0 || pickerIndex < 0 || semanticsIndex < 0 || profileIndex < 0 || navIndex > selectiveIndex || pickerIndex < selectiveIndex || semanticsIndex < pickerIndex || profileIndex < semanticsIndex) throw new Error('Android HOTFIX17 runtime patch order invalid.');
...
if (!html.includes('0.0.97 HOTFIX 17')) throw new Error('Android HOTFIX17 version missing.');
if (!html.includes('ctLooksLikeJwt')) throw new Error('Android HOTFIX17 JWT guard missing.');
if (html.includes('<script src="/')) throw new Error('Android HOTFIX17 still has root script dependencies.');
if (!html.includes("restored = await authRecoveryWithTimeout(restoreSession(), 6500, 'Restauração de sessão');")) throw new Error('Android HOTFIX17 hard restore timeout missing.');
if (!html.includes("localStorage.removeItem('cinetracker_session');")) throw new Error('Android HOTFIX17 poisoned-session cleanup missing.');
console.log(`Android HOTFIX17 series-status bundle prepared with ${scripts.length} inlined scripts; profile classification active; v97 absent.`);
