...
window.__ctAndroidBundle = 'hotfix14-physical-nav-picker-v95-core-inline-authoritative';
...
for (const name of required) {
  if (!html.includes(`data-ct-inline="${name}"`)) throw new Error(`Android HOTFIX14 missing ${name}.`);
}
...
if (navIndex < 0 || selectiveIndex < 0 || pickerIndex < 0 || semanticsIndex < 0 || physicalIndex < 0 || !(navIndex < selectiveIndex && selectiveIndex < pickerIndex && pickerIndex < semanticsIndex && semanticsIndex < physicalIndex)) throw new Error('Android HOTFIX14 runtime patch order invalid.');
if (!html.includes('__ctHotfix12NavPre') || !html.includes('__ctHotfix12PickerGuard')) throw new Error('Android HOTFIX14 navigation/picker legacy protections missing.');
if (!html.includes('__ctHotfix11ImportSync') || !html.includes('__ctHotfix11SettingsBridge')) throw new Error('Android HOTFIX14 import/sync compatibility markers missing.');
if (!html.includes('__ctHotfix13BingersSemantics') || !html.includes('movie_plays') || !html.includes('watch_later_total')) throw new Error('Android HOTFIX14 Bingers semantics marker missing.');
if (!html.includes('__ctHotfix14PhysicalNavPicker') || !html.includes('ct14NativeFileReady') || !html.includes('ct14Navigate')) throw new Error('Android HOTFIX14 physical recovery marker missing.');
...
if (html.includes('patch-v068-v097-observer-guard.js') || html.includes('__ct97ObserverGuard')) throw new Error('Android HOTFIX14 still contains obsolete v97 observer guard.');
if (!html.includes("window.__ctAndroidBundle = 'hotfix14-physical-nav-picker-v95-core-inline-authoritative'")) throw new Error('Android HOTFIX14 bundle marker missing.');
...
if (html.includes('patch-v073-v097-fix7.js') || html.includes('auth-preboot-fix7.js')) throw new Error('Legacy FIX7 found in Android HOTFIX14 bundle.');
if (!html.includes("restored = await authRecoveryWithTimeout(restoreSession(), 6500, 'Restauração de sessão');")) throw new Error('Android HOTFIX14 hard session-restore timeout missing.');
...
console.log(`Android HOTFIX14 physical-recovery bundle prepared with ${scripts.length} inlined scripts; v97 overlay absent.`);