...
const patch='patch-v1195-v0997-route-preload-core.js';
...
for(const dir of dirs){
  ...
  if(!html.includes(anchor))throw new Error(`0.99.7 route preload: v120 anchor missing in ${indexPath}`);
  html=html.replace(anchor,`<script src="/${patch}"></script>${anchor}`);
  await writeFile(indexPath,html,'utf8');
}
console.log('CineTracker Web 0.99.7: route preload core emitted before v120.');
