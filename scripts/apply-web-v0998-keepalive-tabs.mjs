...
const patch='patch-v126-v0998-keepalive-tabs.js';
...
for(const dir of dirs){
  ...
  if(!html.includes(anchor))throw new Error(`0.99.8 keep-alive: v124 anchor missing in ${indexPath}`);
  html=html.replace(anchor,`${anchor}<script src="/${patch}"></script>`);
  await writeFile(indexPath,html,'utf8');
}
console.log('CineTracker Web 0.99.8: keep-alive tabs emitted after v124.');