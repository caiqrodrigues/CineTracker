...
for(const target of targets){
  ...
  if(html.includes('patch-v068-v097.js'))throw new Error(`v0.0.98: v97 overlay must be absent before authoritative layer: ${indexPath}`);
  if(!html.includes('patch-v067-v095.js'))throw new Error(`v0.0.98: stable v95 core missing: ${indexPath}`);
  ...
  for(const name of [pre98,profileName,ui98,compat98]){
  ...
  for(const name of names){
    ...
    if(!html.includes(tag))throw new Error(`v0.0.98: ${name} was not injected: ${indexPath}`);
    await copyFile(resolve(root,'apps/web',name),resolve(target,name));
  }
  const preIndex=html.indexOf(pre98),navIndex=html.indexOf('patch-v085-hotfix15-import-transport.js'),selectiveIndex=html.indexOf('patch-v075-hotfix10-selective.js'),pickerIndex=html.indexOf('patch-v082-hotfix12-picker-guard.js'),semanticsIndex=html.indexOf('patch-v083-hotfix13-bingers-semantics.js'),resilienceIndex=html.indexOf('patch-v087-hotfix16-import-resilience.js'),profileIndex=html.indexOf(profileName),uiIndex=html.indexOf(ui98),compatIndex=html.indexOf(compat98);
  if(preIndex<0||navIndex<0||selectiveIndex<0||pickerIndex<0||semanticsIndex<0||resilienceIndex<0||profileIndex<0||uiIndex<0||compatIndex<0||!(preIndex<navIndex&&navIndex<selectiveIndex&&selectiveIndex<pickerIndex&&pickerIndex<semanticsIndex&&semanticsIndex<resilienceIndex&&resilienceIndex<profileIndex&&profileIndex<uiIndex&&uiIndex<compatIndex))throw new Error(`v0.0.98: runtime patch order invalid: ${indexPath}`);
  if(html.includes('patch-v086-'+'hotfix15-import-retry.js'))throw new Error(`v0.0.98: legacy retry layer still active: ${indexPath}`);
  await writeFile(indexPath,html,'utf8');
}

console.log('CineTracker 0.0.98: authoritative navigation gate emitted first; profile/discover/settings UI emitted last.');
