...
const scope=`
let homeCache=null,profileCache=null,discoverCache=new Map(),ct169DrawerState={showId:456,seasonNo:1};window.__markCalls=0;
function mediaTmdb(x){return Number(x?.id||x?.tmdb_id||0)} async function discoverRows(){return {}} function go(){} function toast(){}
...
`;
const driverScript=`${scope}\n${authority.replaceAll('</script>','<\\/script>')}\n${runtime.replaceAll('</script>','<\\/script>')}\n{let n=0;const t=setInterval(()=>{n++;const b=document.querySelector('[data-ct107-rewatch="episode"][data-ct212-bound="1"]');if(b){clearInterval(t);b.click()}else if(n>40)clearInterval(t)},100)}`;
const html=`<!doctype html><html><head></head><body><div class="version"></div><main data-sports><div class="sports-summary card">10 jogos · 3 favoritos · 4 eventos</div><div class="card" id="keep-event">EVENT CARD PRESERVED</div><section><h2>Histórico recente</h2><div class="media-row" data-media="movie:123"><b>History Movie</b><button data-ct104-rewatch>LEGACY REWATCH</button></div></section><div class="detail-view" data-media="movie:123"><button>✓ ASSISTIDO</button></div><div class="episode-row" data-media="tv:456" data-episode-number="2">E2 · Assistido</div></main><script>${driverScript}</script></body></html>`;
await writeFile(fixture,html,'utf8');
let out='';for(const bin of ['google-chrome','chromium','chromium-browser']){try{out=execFileSync(bin,['--headless','--no-sandbox','--disable-gpu','--virtual-time-budget=5000','--dump-dom','file://'+fixture],{encoding:'utf8',stdio:['ignore','pipe','pipe']});if(out)break}catch{}}
await rm(fixture,{force:true});if(!out)throw new Error('Chrome/Chromium unavailable');
for(const must of ['EVENT CARD PRESERVED','id="ct-f1-v107"','Fixture GP','Fixture Circuit','data-ct107-rewatch="movie"','data-ct107-rewatch="episode"','data-ct212-bound="1"','data-mark-calls="1"','Reassistir 3x','CineTracker • v1.0.7'])if(!out.includes(must)){console.log('REWATCH_DOM',out.match(/<button[^>]*data-ct107-rewatch[^>]*>[^<]*/g));console.log('BODY_MARK',out.match(/data-mark-calls="[^"]+"/g));console.log('R212_STAGE',out.match(/data-ct212-stage="[^"]+"/g));throw new Error('Browser behavior missing '+must)}
