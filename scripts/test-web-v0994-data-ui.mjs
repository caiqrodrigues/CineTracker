...
assert.match(legacy,/const hasAnyHistory=x=>Boolean/,'strict personal discovery history guard missing');
assert.match(legacy,/x\.is_watchlist&&!hasAnyHistory\(x\)/,'strict personal Watchlist exclusion missing');
...
assert.match(legacy,/filter\(x=>Number\(x\.id\)>0&&!blocker\.isBlocked\(x\)\)/,'global discovery exclusion missing');
assert.match(legacy,/aliases\.has\(`\$\{type\}:\*:\$\{n\}`\)/,'year-independent title alias exclusion missing');
assert.match(legacy,/next_episode_to_air/,'calendar stable implementation missing official episode data');
assert.match(legacy,/discoverFilters991\(true\)/,'calendar filters missing');
...
console.log('WEB_0994_DATA_UI_OK history=newest-first discover=strict-behavior calendar=stable settings=unified authority=canonical-aware');
