...
const root=resolve(process.cwd()),group=process.env.CT_ASSERT||'all';
if(!['discover','sports','watchlist','home','all'].includes(group))throw new Error('Unknown CT_ASSERT '+group);
...
const requested=${JSON.stringify(group)};const wait=ms=>new Promise(r=>setTimeout(r,ms));
let homeCache={series:[]};let profileCache={dashboard:[]};let discoverCache=new Map(),sportsCache=null;
function localDay(){return'2026-09-09'}function img(p){return p||''}function go(p){document.body.dataset.go=p}
...
function paintHome(){document.body.dataset.homePaint=(Number(document.body.dataset.homePaint||0)+1).toString()}async function renderProfile(){}function paintDiscover(){}
async function renderDiscover(){document.querySelector('#app').innerHTML='<div data-page="discover" data-discover><div data-discover-content><div class="loader">Carregando</div></div></div>';await wait(180);document.querySelector('[data-discover-content]').innerHTML='<article id="newcard" class="card"><div class="poster" style="background-image:url(x)"></div><b>Novo</b></article>'}
function sportsFiltered(p){return p?.events||[]}function paintSports(){}async function renderSports(){}async function syncSports(){}
...
(async()=>{try{const app=document.querySelector('#app');
 if(requested==='discover'||requested==='all'){
  await wait(60);app.innerHTML='<div data-page="discover" data-discover><div id="legend">REGRA ATIVA Personalizado · respeita histórico, progresso e Watchlist</div><div data-discover-content><div class="row"><article id="dcard" class="card"><div class="poster" style="background-image:url(x)"></div><b>Card</b></article></div></div></div>';await wait(320);
  document.body.dataset.legend=String(!!document.querySelector('#legend'));document.body.dataset.card=String(document.querySelector('#dcard')?.classList.contains('ct127-discover-card'));
  document.querySelector('[data-discover-content]').innerHTML='<div id="oldcontent">OLD-CONTENT</div>';const task=renderDiscover();await wait(50);document.body.dataset.oldDuringLoad=String(!!document.querySelector('#oldcontent'));await task;await wait(240);document.body.dataset.newAfterLoad=String(!!document.querySelector('#newcard'));
  app.innerHTML='<div data-page="discover" data-discover><button class="active" data-discover-tab="top10">Top 10</button><div data-discover-content><article id="topcard" class="card"><div class="poster" style="background-image:url(x)"></div><b>Top</b></article></div></div>';await wait(240);document.body.dataset.topUntouched=String(!document.querySelector('#topcard')?.classList.contains('ct127-discover-card'));
 }
 if(requested==='sports'||requested==='all'){
  await wait(60);app.innerHTML='<div data-sports><div class="event-grid"><article id="sport"><button>Ver eventos</button><button>Eventos</button><button>✓ Marcar assistida</button><button>✓ Marcar como assistido</button></article></div></div>';await wait(320);
  const actions=[...document.querySelectorAll('#sport .ct127-sports-actions .ct127-sport-action')].map(x=>x.textContent.trim());document.body.dataset.sportCount=String(actions.length);document.body.dataset.sportText=actions.join('|');document.body.dataset.sportDuplicates=String([...document.querySelectorAll('#sport button')].filter(x=>/eventos|assist/i.test(x.textContent)).length);
  const dd=sportsFiltered({events:[{sport_slug:'soccer',provider:'thesportsdb',home_name:'A FC',away_name:'B FC',starts_at:'2026-09-09T12:00:00Z'},{sport_slug:'soccer',provider:'api-sports:football',home_name:'A FC',away_name:'B FC',starts_at:'2026-09-09T13:00:00Z'}]});document.body.dataset.sportDedupe=String(dd.length);document.body.dataset.sportProvider=String(dd[0]?.provider||'');document.body.dataset.sportHtml=document.querySelector('#sport')?.innerHTML.replace(/\s+/g,' ').slice(0,800)||'';
 }
 if(requested==='watchlist'||requested==='all'){
  await wait(60);app.innerHTML='<div data-profile><button class="stat"><small>Séries Watchlist</small><b>0</b></button><button class="stat"><small>Filmes Watchlist</small><b>0</b></button></div>';await wait(320);
  document.body.dataset.movies=document.querySelector('[data-ct127-watchlist="movie"] b')?.textContent||'';document.body.dataset.series=document.querySelector('[data-ct127-watchlist="series"] b')?.textContent||'';const modal=await window.__ctV127OpenWatchlist('movie');await wait(180);document.body.dataset.modalCount=modal.querySelector('[data-ct127-count]')?.textContent||'';document.body.dataset.modalRows=String(modal.querySelectorAll('.ct127-watch-row').length);document.body.dataset.localRows=String(modal.querySelectorAll('.ct127-watch-local').length);modal.remove();
 }
 if(requested==='home'||requested==='all'){
...
await writeFile(file,fixture);let out='';for(const bin of ['google-chrome','chromium','chromium-browser'])try{out=execFileSync(bin,['--headless','--no-sandbox','--disable-gpu','--virtual-time-budget=6500','--dump-dom','file://'+file],{encoding:'utf8',timeout:30000,stdio:['ignore','pipe','pipe']});if(out)break}catch{}await rm(dir,{recursive:true,force:true});if(!out)throw new Error('Chromium unavailable');
const groups={discover:['data-done="1"','data-legend="false"','data-card="true"','data-old-during-load="true"','data-new-after-load="true"','data-top-untouched="true"'],sports:['data-done="1"','data-sport-count="2"','data-sport-text="Ver eventos|✓ Marcar como assistido"','data-sport-duplicates="2"','data-sport-dedupe="1"','data-sport-provider="api-sports:football"'],watchlist:['data-done="1"','data-movies="1.358"','data-series="564"','data-modal-count="1.358"','data-modal-rows="1358"','data-local-rows="271"'],home:['data-done="1"','data-home-known="continue:false"','data-home-fresh="continue:false:5"']};
const must=group==='all'?Object.values(groups).flat():groups[group];const body=out.match(/<body[^>]*>/)?.[0]||'NO_BODY';for(const m of must)if(!out.includes(m))throw new Error(`V127 ${group} missing ${m}\n${body}`);if(out.includes('data-err='))throw new Error(`V127 ${group} runtime error\n${body}`);console.log(`V127_BROWSER_OK ${group}`);