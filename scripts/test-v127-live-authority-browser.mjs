...
async function rpc(name){
 if(name!=='cinetracker_watchlist_full_v119')return{};
 const rows=[];for(let i=1;i<=1358;i++)rows.push({media_id:i,media_type:'movie',tmdb_id:i%5===0?0:i,title:'Movie '+String(i).padStart(4,'0'),release_year:2000+i%26,poster_path:i%7===0?null:'/m.jpg',added_at:'2026-09-01T00:00:00Z'});
 for(let i=1;i<=564;i++)rows.push({media_id:2000+i,media_type:'tv',tmdb_id:i%4===0?0:5000+i,title:'Series '+String(i).padStart(4,'0'),release_year:2000+i%26,poster_path:null,added_at:'2026-08-01T00:00:00Z'});
 return{rows,counts:{movie:1358,series:564},count:1922};
}
...
(async()=>{try{
 ...
 // Observer proof: DOM appears only after the runtime was already loaded.
 await wait(40);app.innerHTML='<div data-page="discover" data-discover><div id="legend">REGRA ATIVA Personalizado · respeita histórico, progresso e Watchlist</div><div data-discover-content><div class="row"><article id="dcard" class="card"><div class="poster" style="background-image:url(x)"></div><b>Card</b></article></div></div></div>';
 await wait(120);document.body.dataset.legend=String(!!document.querySelector('#legend'));document.body.dataset.card=String(document.querySelector('#dcard')?.classList.contains('ct127-discover-card'));
 // Anti-flicker: old content must remain visible while a tab's async render is waiting.
 document.querySelector('[data-discover-content]').innerHTML='<div id="oldcontent">OLD-CONTENT</div>';
 const renderTask=renderDiscover();await wait(20);document.body.dataset.oldDuringLoad=String(!!document.querySelector('#oldcontent'));await renderTask;await wait(80);document.body.dataset.newAfterLoad=String(!!document.querySelector('#newcard'));
 // Top 10 is deliberately untouched by the uniform-card pass.
 ...
 // Sports: duplicated actions inserted after boot collapse to one exact action zone.
 ...
 // Provider duplicate reconciliation chooses the stronger source for the same game.
 ...
 // Watchlist: rows without TMDB ids remain in the exact same universe as the counters.
 ...
 // Home known payload and a stale payload revalidated through TMDB both leave Em dia.
...
}catch(e){document.body.dataset.err=String(e?.stack||e);document.body.dataset.done='1'}})();
</script></body></html>`;
await writeFile(file,fixture);
let out='';for(const bin of ['google-chrome','chromium','chromium-browser'])try{out=execFileSync(bin,['--headless','--no-sandbox','--disable-gpu','--virtual-time-budget=4200','--dump-dom','file://'+file],{encoding:'utf8',timeout:30000,stdio:['ignore','pipe','pipe']});if(out)break}catch{}
await rm(dir,{recursive:true,force:true});if(!out)throw new Error('Chromium unavailable');
const must=['data-done="1"','data-legend="false"','data-card="true"','data-old-during-load="true"','data-new-after-load="true"','data-top-untouched="true"','data-sport-count="2"','data-sport-text="Ver eventos|✓ Marcar como assistido"','data-sport-duplicates="2"','data-sport-dedupe="1"','data-sport-provider="api-sports:football"','data-movies="1.358"','data-series="564"','data-modal-count="1.358"','data-modal-rows="1358"','data-local-rows="271"','data-home-known="continue:false"','data-home-fresh="continue:false:5"'];
for(const m of must)if(!out.includes(m))throw new Error('V127 missing '+m+'\n'+out.slice(-8000));
if(out.includes('data-err='))throw new Error('V127 runtime error\n'+out.slice(-8000));
console.log('V127_LIVE_AUTHORITY_BROWSER_OK delayed-dom discover-no-flicker top10-safe sports-deduped watchlist-1358+564 home-aired-unwatched');