...
function makeRuntime(iteration, { storageThrows = false, timeout = false } = {}) {
  ...
  const form = {
    id: 'auth-form', dataset: {},
    closest(sel) { return sel === '#auth-form' ? this : null; },
    querySelector(sel) { return sel === '.auth-submit' || sel === 'button[type="submit"]' ? button : null; }
  };
  ...
  const localStorage = {
    ...
    setItem(key, value) {
      if (storageThrows) throw new Error('quota exceeded');
  ...
  vm.runInContext(`
    ...
    async function restoreSession(){return false;}
    function bindAuth(){}
  `, context);
...
for (let i = 1; i <= 50; i++) {
  ...
  assert.equal(runtime.button.disabled, false, `login ${i} deixou botão bloqueado`);
...

{
  ...
  assert.equal(runtime.button.disabled, false, 'falha de storage deixou loading preso');
  assert.equal(runtime.context.__ctAuthIsLoading, false);
