...
if(html.includes('data-ct-inline="patch-v086-hotfix15-import-retry.js"'))throw new Error('Android inline smoke: obsolete v086 retry layer remains');
const navIndex=html.indexOf('data-ct-inline="patch-v085-hotfix15-import-transport.js"'),selectiveIndex=html.indexOf('data-ct-inline="patch-v075-hotfix10-selective.js"'),pickerIndex=html.indexOf('data-ct-inline="patch-v082-hotfix12-picker-guard.js"'),semanticsIndex=html.indexOf('data-ct-inline="patch-v083-hotfix13-bingers-semantics.js"'),resilienceIndex=html.indexOf('data-ct-inline="patch-v087-hotfix16-import-resilience.js"'),profileIndex=html.indexOf('data-ct-inline="patch-v074-hotfix1-version.js"');
if(navIndex<0||selectiveIndex<0||pickerIndex<0||semanticsIndex<0||resilienceIndex<0||profileIndex<0||!(navIndex<selectiveIndex&&selectiveIndex<pickerIndex&&pickerIndex<semanticsIndex&&semanticsIndex<resilienceIndex&&resilienceIndex<profileIndex))throw new Error('Android inline smoke: HOTFIX18 patch order invalid');
const markers=['__ctHotfix15ImportTransport','ct15RestoreNativeFiles','ct15EnhanceNativePicker','__ctHotfix12PickerGuard','__ctHotfix10Selective','__ctHotfix10Actions','__ctHotfix10NativeBridge','__ctHotfix11ImportSync','__ctHotfix11SettingsBridge','__ctHotfix13BingersSemantics','__ctProfileStatsHotfix17','__ctHotfix18Version'];
for(const marker of markers)if(!html.includes(marker))throw new Error(`Android inline smoke: ${marker} missing`);
if(!html.includes('movie_plays')||!html.includes('episode_plays')||!html.includes('watch_later_total')||!html.includes('not_started_series')||!html.includes('up_to_date_series'))throw new Error('Android inline smoke: Bingers/profile aggregate semantics missing');
...
if(!html.includes('0.0.97 HOTFIX 18'))throw new Error('Android inline smoke: HOTFIX18 version missing');
...
console.log(`Android HOTFIX18 inline smoke OK: ${scripts.length} scripts; native picker; resilient import; profile states; v97 absent.`);
