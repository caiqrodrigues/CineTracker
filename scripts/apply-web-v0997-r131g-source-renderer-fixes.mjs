...
function rowMovie994`);
  ...
  // 2) v126 repeatedly forced Profile back to 4 cards in both JS and !important CSS.
  // Move the source contract itself to 10 and preserve its expansion class.
  const profilePath=resolve(dir,'patch-v126-v0997-video3124-recovery.js');
  let profile=await readFile(profilePath,'utf8');
  if(!profile.includes('nth-child(n+5)'))throw new Error(`r131g: v126 four-card CSS not found in ${profilePath}`);
  profile=profile.replaceAll('nth-child(n+5)','nth-child(n+11)');
  ...
  profile=profile.replace(profilePattern,`function applyTenMore(name){const sec=findProfileSection(name);if(!sec)return;const row=sec.querySelector('.ct120-row,.ct118-row');if(!row)return;row.classList.add('ct126-profile-grid');const cards=[...row.children].filter(x=>x.matches?.('.ct120-card,.ct118-card'));if(!cards.length)return;const signature=String(cards.length);const existing=row.querySelector(':scope > .ct126-more');if(row.dataset.ct126Ten===signature&&(cards.length<=10||existing))return;row.querySelectorAll(':scope > .ct124-more,:scope > .ct122-more-card,:scope > .ct126-more').forEach(x=>x.remove());row.classList.remove('ct126-expanded');cards.forEach((c,i)=>{c.hidden=i>=10});row.dataset.ct126Ten=signature;if(cards.length<=10)return;const more=document.createElement('button');more.type='button';more.className='ct126-more';more.innerHTML=\`<span><b>Ver mais</b><small>+\${cards.length-10}</small></span>\`;let open=false;more.onclick=()=>{open=!open;row.classList.toggle('ct126-expanded',open);cards.forEach((c,i)=>{c.hidden=!open&&i>=10});more.querySelector('b').textContent=open?'Mostrar menos':'Ver mais';more.querySelector('small').textContent=open?'':\`+\${cards.length-10}\`};row.appendChild(more)}
function applyTenActors(){const sec=findProfileSection('Atores Favoritos');if(!sec)return;const row=sec.querySelector('.ct120-actors,.ct118-actors');if(!row)return;row.classList.add('ct126-profile-grid');const cards=[...row.children].filter(x=>x.matches?.('.ct120-actor,.ct118-actor,.ct118-person'));if(!cards.length)return;const signature=String(cards.length);const existing=row.querySelector(':scope > .ct126-more');if(row.dataset.ct126Ten===signature&&(cards.length<=10||existing))return;row.querySelectorAll(':scope > .ct126-more').forEach(x=>x.remove());row.classList.remove('ct126-expanded');cards.forEach((c,i)=>{c.hidden=i>=10});row.dataset.ct126Ten=signature;if(cards.length<=10)return;const more=document.createElement('button');more.type='button';more.className='ct126-more ct126-more-actor';more.innerHTML=\`<span><b>Ver mais</b><small>+\${cards.length-10} atores</small></span>\`;let open=false;more.onclick=()=>{open=!open;row.classList.toggle('ct126-expanded',open);cards.forEach((c,i)=>{c.hidden=!open&&i>=10});more.querySelector('b').textContent=open?'Mostrar menos':'Ver mais';more.querySelector('small').textContent=open?'':\`+\${cards.length-10} atores\`};row.appendChild(more)}
function cleanupProfile(){if(route()!=='profile')return;removeStandaloneHistory();applyTenMore('Séries');applyTenMore('Filmes');applyTenMore('Séries Favoritas');applyTenMore('Filmes Favoritos');applyTenActors()}`);
  if(profile.includes('applyFourMore(')||profile.includes('cards.length-4')||profile.includes('nth-child(n+5)'))throw new Error('r131g: Profile four-card override survived');
  if(!profile.includes("applyTenMore('Séries Favoritas')")||!profile.includes('applyTenActors()')||!profile.includes('nth-child(n+11)')||!profile.includes("row.classList.toggle('ct126-expanded',open)"))throw new Error('r131g: Profile 10+ contract missing');
