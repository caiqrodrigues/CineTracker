...
for(const needle of ["applyTenMore('Séries')","applyTenMore('Filmes')","applyTenMore('Séries Favoritas')","applyTenMore('Filmes Favoritos')",'applyTenActors()','c.hidden=i>=10','cards.length-10','nth-child(n+11)',"row.classList.toggle('ct126-expanded',open)",'row.dataset.ct126Ten===signature']){
  if(!profile.includes(needle))throw new Error(`r131g Profile missing ${needle}`);
}
if(profile.includes('applyFourMore(')||profile.includes('cards.length-4')||profile.includes('nth-child(n+5)'))throw new Error('r131g four-card Profile override survived');
...
console.log('WEB_R131G_OK home=canonical-total+released-missing profile=10-more+css discover=r131+calendar');
