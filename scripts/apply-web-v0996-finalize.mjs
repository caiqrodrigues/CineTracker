...
function mediaCard116(x){
  const id=effectiveId116(x),type=x.media_type==='movie'?'movie':'tv',poster=x.poster_path||x.raw_tmdb?.poster_path||null,meta=[x.release_year||year116(x)||null,type==='movie'?'Filme':isAnime116(x)?'Anime':'Série',progressText116(x)].filter(Boolean).join(' · ');
  return `<article class="ct116-card" data-ct115-heart-bound="1" data-card991="${Number(x.media_id||0)}"><button type="button" class="ct116-card-open" data-open-media991="${type}:${id}"><div class="ct116-poster"${poster?` style="background-image:url('${img116(poster)}')"`:''}></div><div class="ct116-body"><b>${esc116(x.title||'Sem título')}</b><small>${esc116(meta)}</small></div></button>${mediaHeart116(x)}</article>`;
}

function tmdbCard116(x){
  const type=x.media_type==='movie'?'movie':'tv',poster=x.poster_path||x.raw_tmdb?.poster_path||null,episode=x._calendar_episode||null,episodeCode=episode?`S${String(episode.season_number||0).padStart(2,'0')}E${String(episode.episode_number||0).padStart(2,'0')}`:null,meta=[episodeCode||year116(x)||null,type==='movie'?'Filme':isAnime116(x)?'Anime':'Série',score116(x)>0?`★ ${score116(x).toFixed(1)}`:null].filter(Boolean).join(' · ');
  return `<article class="ct116-card" data-ct115-heart-bound="1" data-open-media991="${type}:${Number(x.id)}"><button type="button" class="ct116-card-open"><div class="ct116-poster"${poster?` style="background-image:url('${img116(poster)}')"`:''}></div><div class="ct116-body"><b>${esc116(x.title||x.name||'Sem título')}</b><small>${esc116(meta)}</small></div></button><button type="button" class="ct116-heart" data-ct116-tmdb-heart="${type}:${Number(x.id)}" aria-label="Favoritar">♡</button></article>`;
}
...
async function fetchDiscover116(force=false){
  ...
  discoverBusy116=(async()=>{
    ...
    async function resolvePersonal(kind){
      ...
      const searches=await Promise.all(personalType(kind).slice(0,12).map(async x=>{
        const q=String(x.title||'').replace(/\s*\((?:19|20)\d{2}\)\s*$/,'').trim();if(!q)return null;
        ...
        const s=await safeApi116(`/search/${type}`,params),want=norm116(q),yr=Number(x.release_year||0);
      ...
      return searches.filter(Boolean).sort((a,b)=>Number(b.popularity||0)-Number(a.popularity||0))[0]||null;
    }
    ...
    if(!daily&&!trending.length&&!top.length)throw new Error('Catálogo TMDB temporariamente indisponível');
    const tvCalendar=dash.filter(x=>x.media_type==='tv'&&(x.is_watchlist||x.is_in_progress||x.is_up_to_date)).map(x=>{const rawTmdb=x.raw_tmdb||{},n=rawTmdb.next_episode_to_air,id=effectiveId116(x);if(!id||!n?.air_date)return null;const dt=new Date(n.air_date+'T12:00:00');if(dt<today||dt>future45)return null;return {...toTmdb116(x),id,media_type:'tv',first_air_date:n.air_date,_calendar_episode:{season_number:Number(n.season_number||0),episode_number:Number(n.episode_number||0),name:n.name||''}}}).filter(Boolean);
    ...
    const newTvCalendar=upcomingTv.filter(x=>{const d=x.first_air_date;if(!d)return false;const dt=new Date(d+'T12:00:00');return dt>=today&&dt<=future45});
    const calendar=uniq116([...movieCalendar,...tvCalendar,...newTvCalendar]).sort((a,b)=>String(a.release_date||a.first_air_date||'9999').localeCompare(String(b.release_date||b.first_air_date||'9999'))).slice(0,80);
...
for(const dir of dirs){
  ...
  src=replaceRange(src,'function mediaCard116(x){','function tmdbCard116(x){',mediaCard116.toString(),'local media poster fallback');
  src=replaceRange(src,'function tmdbCard116(x){','async function ensureMedia116',tmdbCard116.toString(),'calendar episode card');
  src=replaceRange(src,'async function fetchDiscover116(force=false){','function discoverTabs116()',fetchDiscover116.toString(),'strict fast discover');
...
console.log('CineTracker 0.99.6 final: strict/fast Discover + poster/actor/profile/season repairs emitted.');
