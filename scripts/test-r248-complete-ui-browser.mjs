...
try{
 ...
 await writeFile(html,`<!doctype html><html><head><meta charset="utf-8"><style>${css}</style></head><body><div id="app"><div data-home></div><div id="p-sports" data-sports><button>Eventos</button></div><div id="p-profile" data-profile><section class="panel"><h2>Estatísticas</h2><div class="stats-grid"><div class="stat" id="s-filmes">Filmes</div></div></section><section class="panel"><h2>Estatísticas de esporte</h2><div><div class="stat" id="sport-stat">Esporte</div></div></section></div><div id="p-details"><div class="season-tabs"><div style="width:1200px">Temporadas</div></div><div class="related-scroll"><div style="width:1200px">Relacionados</div></div><div class="episode-graph"><div style="width:1200px">Gráfico</div></div></div></div><script>
 ...
 setTimeout(async()=>{try{
  ...
  const events=[mk(0,14),mk(1,14),mk(-1,14),mk(-2,14,true),mk(-3,14,false,true),mk(-4,14)];
 ...
 for(const x of ['data-done="1"','data-raw="caught_up:true:400"','data-current="caught_up:true"','data-seriesevent="continue:false"','data-filters="1,3,1,1"','data-tabs="Próximos|Anteriores|Favoritos|Assistidos"','data-eventos="false"','data-f1tabs="6"','data-f1hidden="true"','data-stats="1"','data-sportstat="true"'])if(!out.includes(x))throw new Error('R248 UI missing '+x+'\n'+(out.match(/<body[^>]*>/)?.[0]||''));
 const rails=Number((out.match(/data-rails="(\d+)"/)||[])[1]||0),errors=(out.match(/data-errors="([^"]*)"/)||[])[1]||'';if(rails<3)throw new Error('R248 local rails missing '+rails);if(errors)throw new Error('R248 UI errors '+errors);
 console.log('R248_UI_OK backlog-preserved current-new series-event sports=4 f1=6 profile=1 rails='+rails);
