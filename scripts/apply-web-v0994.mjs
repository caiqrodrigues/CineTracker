...
const postName = 'patch-v102-v0994-mobile-nav-fix.js';
...
for (const target of targets) {
  ...
  html = html.replace('</body>', `${finalTag}${postTag}</body>`);
  if (html.indexOf(preTag) >= html.indexOf(anchor)) throw new Error(`Web 0.99.4: pre-gate order invalid: ${indexPath}`);
  if (html.indexOf(postTag) <= html.indexOf(finalTag)) throw new Error(`Web 0.99.4: post-gate order invalid: ${indexPath}`);
  await writeFile(indexPath, html, 'utf8');
  for (const name of [preName, finalName, postName]) await copyFile(resolve(root, 'apps/web', name), resolve(target, name));
}
console.log('CineTracker Web 0.99.4: 0.99.2/0.99.3 navigation runtimes removed; canonical desktop/mobile 0.99.4 runtime emitted.');
