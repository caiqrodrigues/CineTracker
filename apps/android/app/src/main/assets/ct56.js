...
window.ct48Navigate=(target)=>{
  ...
  }catch(e){try{return typeof priorNavigate==='function'?priorNavigate(target):false}catch{return false}}
};

function fixSettings(){
  if(currentView()!=='settings')return;
  const footer=$('#ct49-build-footer');if(footer)footer.textContent='CineTracker Android • build 0.0.66';
  $$('*').filter(el=>el.children.length===0).forEach(el=>{
    const t=(el.textContent||'').trim();
    if(/^CineTracker Web\s+0\.\d+\.\d+\s*[•·-]\s*Android build\s+0\.0\.\d+$/i.test(t))el.textContent='CineTracker Web 0.4.8 • Android build 0.0.66';
    else if(/^CineTracker Android\s*[•·-]?\s*build\s+0\.0\.\d+$/i.test(t))el.textContent='CineTracker Android • build 0.0.66';
  });
}
