...
function version(){if(typeof view==='undefined'||view!=='settings')return;const host=$('.content')||$('#app');if(!host)return;let f=$('#ct82-version');if(!f){f=document.createElement('div');f.id='ct82-version';f.style.cssText='margin:28px 0 8px;text-align:center;color:#71808b;font-size:11px';host.appendChild(f)}$$('#ct66-version,.ct53w-version,.ct54-version',host).forEach(x=>x.remove());f.textContent='CineTracker Android • versão 0.0.82'}
const old=window.render;if(typeof old==='function'&&!window.__ct68Render){window.__ct68Render=old;window.render=function(){const out=window.__ct68Render();smooth();setTimeout(version,0);return out}}
setTimeout(()=>{smooth();version();void preload()},0);
