...
window.__ctAndroidBuild='0.0.64';
...
.ct47-card{min-height:0!important}.ct47-card .ct47-body{min-height:0!important;padding-bottom:10px!important}.ct54-hide{display:none!important}
`;
...
function normalizeHomeProgress(){
  $$('.ct48-home-card').forEach(card=>{const meta=$('.ct48-home-meta',card);if(!meta)return;const text=(meta.textContent||'').replace(/\s+/g,' ');const kind=(text.match(/\b(ANIME|SÉRIE)\b/i)||[])[1]||'SÉRIE';const prog=(text.match(/\b\d{1,4}\s*\/\s*\d{1,4}\b/)||[])[0];const faltam=(text.match(/Faltam\s+\d{1,4}\s+episódios?/i)||[])[0];const rating=(text.match(/★\s*\d{1,2}(?:[.,]\d)?/)||[])[0];const bits=[kind.toUpperCase(),prog,faltam,rating].filter(Boolean);if(bits.length>=2)meta.textContent=bits.join(' · ')});
}
async function detailState(){
  if(!active.id||!$('.ct50-actions'))return;let seen=false,following=false,watchlist=false;
  try{const [over,cont]=await Promise.all([sbApi('media_overrides?select=state,media:media(tmdb_id,media_type)&limit=5000').catch(()=>[]),active.type==='tv'?sbRpc('cinetracker_continue_items_v2',{}).catch(()=>[]):Promise.resolve([])]);for(const r of over||[]){const m=r?.media;if(!m||Number(m.tmdb_id)!==active.id||m.media_type!==active.type)continue;if(['AlreadySeen','Completed'].includes(r.state))seen=true;if(r.state==='AddedToWatchlist'||r.state==='WatchLater')watchlist=true}if(active.type==='tv')following=(cont||[]).some(r=>Number(r.tmdb_id)===active.id&&['following','in_progress'].includes(String(r.status||'').toLowerCase()))}catch{}
  ...
  const watchBtn=$('[data-ct50-watch]');if(watchBtn){watchBtn.classList.remove('ct54-following');if(following){watchBtn.textContent='Acompanhando';watchBtn.classList.add('ct54-following');watchBtn.disabled=true}else{watchBtn.disabled=false;watchBtn.textContent=watchlist?'Na Watchlist':'＋ Watchlist'}}
}
...
function fixProfileLoading(){if(typeof view==='undefined'||view!=='profile')return;const hasChart=!!($('.ct41-wrap')||$('.ct52-profile-chart'));if(!hasChart)return;$$('*').filter(el=>el.children.length===0&&/^Carregando perfil\.\.\.$/i.test((el.textContent||'').trim())).forEach(el=>{const p=el.closest('.panel,.card,section,div');if(p&&p!==$('.content'))p.remove();else el.remove()});const body=$('#ct43-screen-body');if(body)body.style.removeProperty('display');const full=$('.ct43-full');if(full)full.style.removeProperty('display')}
function unifyDetailRatings(){const hero=$('.ct47-hero');if(!hero)return;const official=$('.ct50-ratingbar');if(!official)return;for(const el of [...hero.parentElement.children]){if(el===official||el.classList.contains('ct50-actions')||el===hero)continue;const t=(el.textContent||'').trim();if(/TMDB\s*\d/i.test(t)&&/IMDb|temporadas|episódios|avaliações/i.test(t))el.classList.add('ct54-hide')}}
...
rememberActive();let q=false;new MutationObserver(()=>{if(q)return;q=true;requestAnimationFrame(()=>{q=false;apply()})}).observe($('#app')||document.documentElement,{childList:true,subtree:true});setTimeout(apply,80);setTimeout(apply,450);setTimeout(apply,1200);
})();
