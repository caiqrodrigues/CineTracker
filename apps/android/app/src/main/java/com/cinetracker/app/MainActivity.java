...
public class MainActivity extends Activity {
    ...
    private static final String APP_VERSION = "0.0.89";
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        if(savedInstanceState==null){String separator=BuildConfig.WEB_URL.contains("?")?"&":"?";webView.loadUrl(BuildConfig.WEB_URL+separator+"android=1&ui=phone&apk=89");} else {webView.restoreState(savedInstanceState);webView.postDelayed(()->{applyAndroidBase();applyStableModules();},120);}
    }
    ...
    private void navigate(String target){String js="(function(){try{var t='"+target+"';if(window.ct89Navigate&&window.ct89Navigate(t))return true;if(window.ct88Navigate&&window.ct88Navigate(t))return true;view=t;if(typeof render==='function'){render();window.scrollTo(0,0);return true;}if(window.ct66Navigate)return !!window.ct66Navigate(t);return false;}catch(e){return false;}})();";webView.evaluateJavascript(js,null);}
    private void applyAndroidBase(){if(webView==null)return;String js="(function(){var m=document.querySelector('meta[name=viewport]');if(!m){m=document.createElement('meta');m.name='viewport';document.head.appendChild(m);}m.content='width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no';if(!document.getElementById('ct89-base')){var s=document.createElement('style');s.id='ct89-base';s.textContent='html,body{width:100%!important;max-width:100%!important;overflow-x:hidden!important;background:#090909!important;-webkit-text-size-adjust:100%!important}body{margin:0!important}.app{display:block!important;width:100%!important;min-width:0!important}.sidebar,.mobile-nav,.cloud-bar{display:none!important}.content{box-sizing:border-box!important;width:100%!important;max-width:none!important;margin:0!important;padding:14px 12px 20px!important;overflow-x:hidden!important}.toast{left:12px!important;right:12px!important;bottom:12px!important;max-width:none!important}';document.head.appendChild(s);}window.__ctAndroidBuild='0.0.89';})();";webView.evaluateJavascript(js,null);}
    private void applyStableModules(){String[] assets={"ct47.js","ct66.js","ct67-profile-history.js","ct68-final.js","ct70-home-084.js","ct71-cast-profile-084.js","ct72-v085.js","ct74-v086-final.js","ct75-v087.js","ct76-v088.js","ct77-v089.js"};for(String asset:assets)applyAsset(asset);}
