...
public class MainActivity extends Activity {
    ...
    private static final String BUNDLED_INDEX = "hotfix2/index.html";
    private WebView webView;
    private ValueCallback<Uri[]> fileChooserCallback;
    private byte[] pendingExportBytes;
    private String pendingExportName;
    private String pendingExportMime;
    private boolean remoteFallbackTriggered = false;
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        webView = findViewById(R.id.webview); webView.setVisibility(View.VISIBLE); webView.setLayerType(View.LAYER_TYPE_HARDWARE, null);
        ...
        webView.setWebChromeClient(new WebChromeClient() {@Override public boolean onShowFileChooser(WebView view, ValueCallback<Uri[]> callback, FileChooserParams params) {if (fileChooserCallback != null) fileChooserCallback.onReceiveValue(null); fileChooserCallback = callback; Intent intent = new Intent(Intent.ACTION_OPEN_DOCUMENT); intent.addCategory(Intent.CATEGORY_OPENABLE); intent.setType("*/*"); intent.putExtra(Intent.EXTRA_MIME_TYPES, new String[]{"text/csv", "application/csv", "application/zip", "application/x-zip-compressed", "application/json", "application/octet-stream"}); intent.putExtra(Intent.EXTRA_ALLOW_MULTIPLE, true); startActivityForResult(intent, FILE_CHOOSER_REQUEST); return true;}});
        webView.setWebViewClient(new WebViewClient() {
            @Override public boolean shouldOverrideUrlLoading(WebView view, WebResourceRequest request) {Uri uri=request.getUrl(); String host=uri.getHost()==null?"":uri.getHost(); if(host.equals("mycinetracker.vercel.app")||host.endsWith("supabase.co"))return false; startActivity(new Intent(Intent.ACTION_VIEW,uri)); return true;}
            @Override public void onPageFinished(WebView view,String url){super.onPageFinished(view,url);applyAndroidBase();CookieManager.getInstance().flush();verifyStartupRendered();}
            @Override public void onReceivedError(WebView view, WebResourceRequest request, WebResourceError error){super.onReceivedError(view,request,error);if(request.isForMainFrame()){if(!remoteFallbackTriggered)loadRemoteFallback();else showStartupError("Não foi possível carregar o CineTracker. Verifique sua conexão e tente novamente.");}}
        });
        bindNativeNavigation(); requestNotificationPermission(); loadBundledHotfix2();
    }
    private String webBaseUrl(){String base=BuildConfig.WEB_URL==null?"https://mycinetracker.vercel.app":BuildConfig.WEB_URL.trim();return base.endsWith("/")?base:base+"/";}
    private String readAssetText(String asset) throws Exception {try(InputStream in=getAssets().open(asset);ByteArrayOutputStream out=new ByteArrayOutputStream()){byte[] buf=new byte[8192];int read;while((read=in.read(buf))!=-1)out.write(buf,0,read);return out.toString(StandardCharsets.UTF_8.name());}}
    private void loadBundledHotfix2(){try{String html=readAssetText(BUNDLED_INDEX);String base=webBaseUrl();String history=base+"?android=1&ui=phone&apk="+BuildConfig.VERSION_CODE+"&release=hotfix2";webView.loadDataWithBaseURL(base,html,"text/html","UTF-8",history);}catch(Exception error){loadRemoteFallback();}}
    private void verifyStartupRendered(){if(startupVerified||webView==null)return;webView.postDelayed(()->{if(startupVerified||webView==null)return;String js="(function(){try{var a=document.getElementById('app');var text=(a&&a.innerText||'').trim();var ok=!!(document.querySelector('#auth-form')||document.querySelector('.content')||text.length>20);return ok&&window.__ctAuthRecovery==='v97-base';}catch(e){return false;}})();";webView.evaluateJavascript(js,value->{if("true".equals(value)){startupVerified=true;}else if(!remoteFallbackTriggered){loadRemoteFallback();}else{showStartupError("O CineTracker abriu, mas a interface não foi renderizada. Instale uma versão posterior ao HOTFIX 2.");}});},1600);}
    private void loadRemoteFallback(){if(webView==null||remoteFallbackTriggered)return;remoteFallbackTriggered=true;startupVerified=false;String base=webBaseUrl();String separator=base.contains("?")?"&":"?";webView.loadUrl(base+separator+"android=1&ui=phone&apk="+BuildConfig.VERSION_CODE+"&release=hotfix2-fallback");}
    private void showStartupError(String message){if(webView==null)return;startupVerified=true;String safe=message.replace("&","&amp;").replace("<","&lt;").replace(">","&gt;");String html="<!doctype html><html><head><meta name='viewport' content='width=device-width,initial-scale=1'></head><body style='margin:0;background:#090909;color:#f4f4f5;font-family:sans-serif;display:grid;place-items:center;min-height:100vh'><div style='max-width:420px;padding:28px;text-align:center'><h2 style='color:#d6b55b'>CineTracker</h2><p>"+safe+"</p></div></body></html>";webView.loadDataWithBaseURL(null,html,"text/html","UTF-8",null);}
