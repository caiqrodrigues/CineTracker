...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        settings.setUserAgentString(settings.getUserAgentString() + " CineTrackerAndroid/0.0.21");
        ...
        if (savedInstanceState == null) {
            ...
            webView.loadUrl(BuildConfig.WEB_URL + separator + "android=1&ui=phone&apk=21");
        } else {
    ...
    private String androidRuntimeScript() {
        return "(function(){" +
                ...
                "function paintExistingPosters(){try{document.querySelectorAll('.card').forEach(function(card){var p=card.querySelector('.poster');if(!p)return;var mid=card.dataset.mediaId||'';var item=null;try{item=typeof mediaRegistry!=='undefined'?mediaRegistry.get(mid):null;}catch(e){}var posterUrl=item&&item.posterUrl;if(posterUrl&&(!p.style.backgroundImage||p.style.backgroundImage==='none')){p.style.backgroundImage='linear-gradient(to top,rgba(0,0,0,.55),rgba(0,0,0,.02)),url(\"'+posterUrl+'\")';p.style.backgroundSize='cover';p.style.backgroundPosition='center 18%';}var h=card.querySelector('h3');if(h&&item&&item.title&&(!h.textContent.trim()||/^tmdb\\s*#/i.test(h.textContent.trim())))h.textContent=item.title;});}catch(e){}}" +
                ...
                "window.ctAndroid21=function(){fixWatchlistPicker();markTrios();paintExistingPosters();removeHomeCalendar();document.querySelectorAll('.cloud-bar').forEach(function(x){x.remove();});};" +
                "window.ctAndroid21();setTimeout(window.ctAndroid21,120);setTimeout(window.ctAndroid21,600);" +
                "})();";
    }
