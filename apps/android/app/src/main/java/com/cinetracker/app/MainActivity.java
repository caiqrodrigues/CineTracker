...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        settings.setUserAgentString(settings.getUserAgentString() + " CineTrackerAndroid/0.0.18");
        ...
        if (savedInstanceState == null) {
            ...
            webView.loadUrl(BuildConfig.WEB_URL + separator + "android=1&ui=phone&apk=18");
        } else {
    ...
    private void navigateView(String directView, String... terms) {
        ...
        words.append(']');
        String direct = directView == null ? "" : "try{view='" + directView + "';render();window.scrollTo(0,0);if(window.ctAndroid18)window.ctAndroid18();return true;}catch(e){}";
        String js = "(function(){" + direct +
                ...
                "for(var i=0;i<els.length;i++){var t=((els[i].textContent||'').trim()).toLowerCase();for(var j=0;j<words.length;j++){if(t.indexOf(words[j])>=0){els[i].click();window.scrollTo(0,0);setTimeout(function(){if(window.ctAndroid18)window.ctAndroid18();},0);return true;}}}return false;})();";
    ...
    private String androidRuntimeScript() {
        return "(function(){" +
                ...
                "if(!document.getElementById('ct-android18-style')){var s=document.createElement('style');s.id='ct-android18-style';s.textContent=`" +
                ...
                "function fixWatchlistPicker(){try{if(typeof currentWatchlistPicks!=='function'||typeof watchlist==='undefined'||typeof mediaRegistry==='undefined')return;if(window.__ct18Picker)return;window.__ct18Picker=1;currentWatchlistPicks=function(){var wanted=['FILME','SÉRIE','ANIME'];var ids=Array.from(watchlist||[]);var all=ids.map(function(id){return mediaRegistry.get(id);}).filter(Boolean);return wanted.map(function(type){return all.find(function(item){return normalizeType(item)===type;});}).filter(Boolean);};if(typeof view!=='undefined'&&view==='home'&&typeof render==='function')render();}catch(e){}}" +
                ...
                "window.ctAndroid18=function(){fixWatchlistPicker();markTrios();paintExistingPosters();document.querySelectorAll('.cloud-bar').forEach(function(x){x.remove();});};" +
                "window.ctAndroid18();setTimeout(window.ctAndroid18,120);setTimeout(window.ctAndroid18,600);" +
                "})();";
    }
