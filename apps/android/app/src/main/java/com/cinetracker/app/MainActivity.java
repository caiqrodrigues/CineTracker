...
public class MainActivity extends Activity {
    ...
    private static final String LOCAL_WEB_URL = "https://appassets.androidplatform.net/assets/hotfix1/index.html";
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        assetLoader = new WebViewAssetLoader.Builder().setDomain("appassets.androidplatform.net").addPathHandler("/assets/", new WebViewAssetLoader.AssetsPathHandler(this)).build();
        ...
        webView.setWebViewClient(new WebViewClient() {
            @Override public WebResourceResponse shouldInterceptRequest(WebView view, WebResourceRequest request) {WebResourceResponse local=assetLoader.shouldInterceptRequest(request.getUrl());return local!=null?local:super.shouldInterceptRequest(view,request);}
            @Override public boolean shouldOverrideUrlLoading(WebView view, WebResourceRequest request) {Uri uri=request.getUrl(); String host=uri.getHost()==null?"":uri.getHost(); if(host.equals("appassets.androidplatform.net")||host.equals("mycinetracker.vercel.app")||host.endsWith("supabase.co"))return false; startActivity(new Intent(Intent.ACTION_VIEW,uri)); return true;}
            @Override public void onPageFinished(WebView view,String url){super.onPageFinished(view,url);applyAndroidBase();CookieManager.getInstance().flush();webView.postDelayed(()->webView.setVisibility(View.VISIBLE),150);}
        });
        bindNativeNavigation(); requestNotificationPermission(); webView.postDelayed(()->webView.setVisibility(View.VISIBLE),1200);
        if(savedInstanceState==null){webView.loadUrl(LOCAL_WEB_URL+"?android=1&ui=phone&apk="+BuildConfig.VERSION_CODE+"&release=hotfix1");} else {webView.restoreState(savedInstanceState);webView.postDelayed(this::applyAndroidBase,120);}
    }
