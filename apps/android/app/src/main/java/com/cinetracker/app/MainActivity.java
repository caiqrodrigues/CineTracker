...
public class MainActivity extends Activity {
    ...
    private static final String AUTH_PREFS = "cinetracker_auth";
    private static final String AUTH_SESSION_JSON = "auth_session_json";
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        if(savedInstanceState==null){String separator=BuildConfig.WEB_URL.contains("?")?"&":"?";webView.loadUrl(BuildConfig.WEB_URL+separator+"android=1&ui=phone&apk="+BuildConfig.VERSION_CODE+"&fix=7&authrev=7b");} else {webView.restoreState(savedInstanceState);webView.postDelayed(()->{applyAndroidBase();applyStableModules();},120);}
    }
    ...
    public class AndroidBridge{
        ...
        @JavascriptInterface public void saveAuthSession(String json){try{JSONObject obj=new JSONObject(json==null?"{}":json);if(obj.optString("access_token","").isEmpty())return;getSharedPreferences(AUTH_PREFS,Context.MODE_PRIVATE).edit().putString(AUTH_SESSION_JSON,json).apply();saveSession(json);}catch(Exception ignored){}}
        @JavascriptInterface public String getAuthSession(){try{return getSharedPreferences(AUTH_PREFS,Context.MODE_PRIVATE).getString(AUTH_SESSION_JSON,"");}catch(Exception ignored){return "";}}
        @JavascriptInterface public void clearAuthSession(){try{getSharedPreferences(AUTH_PREFS,Context.MODE_PRIVATE).edit().remove(AUTH_SESSION_JSON).apply();}catch(Exception ignored){}}
