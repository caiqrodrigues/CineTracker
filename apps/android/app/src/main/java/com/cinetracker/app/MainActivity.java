...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        webView.setWebViewClient(new WebViewClient() {
            @Override public boolean shouldOverrideUrlLoading(WebView view, WebResourceRequest request) {
                ...
                if (host.equals("mycinetracker.vercel.app") || host.endsWith("supabase.co") || host.equals("image.tmdb.org")) return false;
    ...
    private void openSettings() {
        String js = "(function(){" +
                "var b=document.getElementById('ct42-settings-bridge')||document.getElementById('ct40-settings-target');if(b){b.click();window.scrollTo(0,0);setTimeout(function(){if(window.ctAndroid21)window.ctAndroid21();},0);return true;}" +
                "try{view='settings';render();var t=(document.body.textContent||'').toLowerCase();if(t.indexOf('segurança e acesso')>=0||t.indexOf('importar e exportar')>=0){window.scrollTo(0,0);return true;}}catch(e){}" +
                "try{view='account';render();window.scrollTo(0,0);return true;}catch(e){}return false;})();";
    ...
    private void navigateView(String directView, String... terms) {
        ...
        words.append(']');
        String direct = directView == null ? "" : "try{view='" + directView + "';render();window.scrollTo(0,0);if(window.ctAndroid21)window.ctAndroid21();return true;}catch(e){}";
        String js = "(function(){" + direct +
                ...
                "for(var i=0;i<els.length;i++){var t=((els[i].textContent||'').trim()).toLowerCase();for(var j=0;j<words.length;j++){if(t.indexOf(words[j])>=0){els[i].click();window.scrollTo(0,0);setTimeout(function(){if(window.ctAndroid21)window.ctAndroid21();},0);return true;}}}return false;})();";
        webView.evaluateJavascript(js, null);
    }
    ...
    private String androidRuntimeScript() {
        return "(function(){" +
                ...
                "function fixWatchlistPicker(){try{if(document.querySelector('.auth-page'))return;if(typeof currentWatchlistPicks!=='function'||typeof watchlist==='undefined'||typeof mediaRegistry==='undefined')return;if(window.__ct21Picker)return;window.__ct21Picker=1;currentWatchlistPicks=function(){var wanted=['FILME','SÉRIE','ANIME'];var ids=Array.from(watchlist||[]);var all=ids.map(function(id){return mediaRegistry.get(id);}).filter(Boolean);return wanted.map(function(type){return all.find(function(item){return normalizeType(item)===type;});}).filter(Boolean);};if(typeof view!=='undefined'&&view==='home'&&typeof render==='function')render();}catch(e){}}" +
                "function paintExistingPosters(){try{if(document.querySelector('.auth-page'))return;document.querySelectorAll('.card,.feature,.ct38-card,.ct30-fav,.ct30-history-card').forEach(function(card){var p=card.querySelector('.poster,.tmdb-poster,.ct38-poster,.ct30-fav-poster,.ct30-history-poster');if(!p)return;var mid=card.dataset.mediaId||'';var item=null;try{item=typeof mediaRegistry!=='undefined'?mediaRegistry.get(mid):null;}catch(e){}var posterUrl=item&&item.posterUrl;if(posterUrl&&(!p.style.backgroundImage||p.style.backgroundImage==='none')){p.style.backgroundImage='url(\"'+posterUrl+'\")';p.style.backgroundSize='cover';p.style.backgroundPosition='center 18%';}var h=card.querySelector('h3,h2,.ct38-title,.ct30-fav-body strong,.ct30-history-body strong');if(h&&item&&item.title&&(!h.textContent.trim()||/^tmdb\\s*#/i.test(h.textContent.trim())))h.textContent=item.title;});}catch(e){}}" +
                "function fastPosters(){try{if(document.querySelector('.auth-page'))return;window.__ct27cache=window.__ct27cache||{};var cards=[].slice.call(document.querySelectorAll('.card,.feature,.ct38-card,.ct30-fav,.ct30-history-card'));var used=0;cards.forEach(function(card){if(used>=8||card.dataset.ct27==='1')return;var r=card.getBoundingClientRect();if(r.bottom<-150||r.top>innerHeight+250)return;var p=card.querySelector('.poster,.tmdb-poster,.ct38-poster,.ct30-fav-poster,.ct30-history-poster');if(!p)return;var bg=p.style.backgroundImage||'';var h=card.querySelector('h3,h2,.ct38-title,.ct30-fav-body strong,.ct30-history-body strong');var title=(h&&h.textContent||'').trim();if(bg&&bg!=='none'&&bg.indexOf('tmdb-image')<0&&!/^tmdb\\s*#/i.test(title)){card.dataset.ct27='1';return;}var mid=card.dataset.mediaId||'';var id=Number(card.dataset.tmdbId||card.dataset.ct29Id||card.dataset.ct30Id||0),type=card.dataset.tmdbType||card.dataset.ct29Type||card.dataset.apiType||'';try{var it=typeof mediaRegistry!=='undefined'?mediaRegistry.get(mid):null;if(it){id=id||Number(it.tmdbId||0);type=type||it.apiType||it.type||'';}}catch(e){}var m=mid.match(/^tmdb-(movie|tv)-(\\d+)$/);if(!id&&m){type=m[1];id=Number(m[2]);}if(!id)return;type=(String(type).toLowerCase().indexOf('movie')>=0||String(type).toLowerCase().indexOf('filme')>=0)?'movie':'tv';card.dataset.ct27='1';used++;var key=type+':'+id;var apply=function(d){if(!d)return;var real=d.title||d.name||title;if(d.poster_path){p.style.backgroundImage='url(https://image.tmdb.org/t/p/w342'+d.poster_path+')';p.style.backgroundSize='cover';p.style.backgroundPosition='center 18%';}if(h&&real&&(!title||/^tmdb\\s*#/i.test(title)))h.textContent=real;};if(window.__ct27cache[key]){apply(window.__ct27cache[key]);return;}var u=new URL(SUPABASE_URL+'/functions/v1/tmdb-proxy');u.searchParams.set('path','/'+type+'/'+id);u.searchParams.set('language','pt-BR');fetch(u,{headers:authHeaders()}).then(function(x){return x.ok?x.json():null}).then(function(d){if(d){window.__ct27cache[key]=d;apply(d)}}).catch(function(){card.dataset.ct27='0'});});}catch(e){}}" +
                "function settingsBuild(){try{var text=(document.body.textContent||'').toLowerCase();if(text.indexOf('segurança e acesso')<0&&text.indexOf('importar e exportar')<0)return;if(document.getElementById('ct27-build'))return;var host=document.querySelector('.content')||document.getElementById('app');if(!host)return;var f=document.createElement('div');f.id='ct27-build';f.className='ct27-build';f.textContent='CineTracker Android • build 0.0.27';host.appendChild(f);}catch(e){}}" +
                ...
                "window.ctAndroid21=function(){if(document.querySelector('.auth-page'))return;fixWatchlistPicker();markTrios();paintExistingPosters();fastPosters();settingsBuild();removeHomeCalendar();document.querySelectorAll('.cloud-bar').forEach(function(x){x.remove();});};" +
                "window.ctAndroid21();setTimeout(window.ctAndroid21,120);setTimeout(window.ctAndroid21,500);" +
                "})();";
    }
