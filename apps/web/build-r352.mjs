...
let [html,js,css,sw,releaseRaw,runtime]=await Promise.all([
 ...
 readFile(resolve(root,'runtime-r352-foryou-single-slot-actions.js'),'utf8')
]);
...
/* Critical fix: a successful Visto/Watchlist must NOT refetch/repaint all Pra Você rows.
   The clicked slot already changed optimistically in r350 before backend persistence. */
js=once(js,
 "function afterAction351(ok){\n if(ok!==false)void restoreWatchlist351(true);\n}",
 "function afterAction351(ok){return ok!==false;}",
 'disable post-success global watchlist restore'
);
...
css+='\n/* CineTracker Web 1.0.143 r352 — Visto/Watchlist replace only clicked Pra Você slot; no post-success global restore. */\n';

const prev=JSON.parse(releaseRaw),release={
 ...prev,
 version:'1.0.143',revision:'r352-official-1.0.143',base:'r351-production',
 scope:'discover-foryou-single-slot-actions-only',
 discover_foryou_actions:'clicked-slot-optimistic+single-backend-call+no-global-success-reload',
 discover_foryou_watch_section:'startup-restore-only-when-missing',
 discover_foryou_swap:'unchanged-r351',
...
await Promise.all([
 writeFile(resolve(dist,'app-v352.js'),js),
 writeFile(resolve(dist,'app-v352.css'),css),
...
console.log('WEB_R352_READY Visto/Watchlist keep every non-clicked Pra Você slot unchanged after persistence');
