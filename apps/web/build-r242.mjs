...
for(const x of ["window.__ctR242HomeAdditive='preview-first-movie-metadata'","window.__ctR242Scope='home-only'","window.__ctR242Safety='no-renderer-override'","cinetracker_home_preview_v1","ct242MetaText","ct242DecorateMovies"])if(!patch.includes(x))throw new Error('r242 patch missing '+x);
if(/(?:^|\n)\s*(?:renderHome|paintHome)\s*=/.test(patch))throw new Error('r242 must not replace Home renderers');
...
await Promise.all([
  ...
  writeFile(resolve(dist,'release.json'),JSON.stringify({version:'1.0.33',revision:'r242-official-1.0.33',base:'r239-official-1.0.31',scope:'home-only',home:'cache-or-compact-preview-before-full-authority',movie_cards:'year+runtime+up-to-3-genres',renderer_override:false,android:'unchanged-1.0.20',generated_at:new Date().toISOString()},null,2),'utf8')
]);
await Promise.all([rm(resolve(dist,'app-v239.js'),{force:true}),rm(resolve(dist,'app-v239.css'),{force:true})]);
console.log('WEB_1_0_33_READY r242 home=safe-additive-preview movie=year+runtime+genres renderer-override=false');
