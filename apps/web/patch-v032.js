...
let pending=false;
const schedule=root=>{if(pending)return;pending=true;requestAnimationFrame(()=>{pending=false;run(root||document);});};
const obs=new MutationObserver(ms=>{for(const m of ms){for(const n of m.addedNodes){if(n.nodeType===1){schedule(n);return;}}}});obs.observe(document.documentElement,{childList:true,subtree:true});
window.addEventListener('pageshow',()=>schedule(document));setTimeout(()=>schedule(document),0);
})();
