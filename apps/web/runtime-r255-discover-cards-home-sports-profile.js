...
function eligible255(x,k,{allowWatch=false,fresh=false}={}){const id=tmdb255(x),yr=n255(year255(x)),score=score255(x),t=norm255(title255(x));if(!id||!poster255(x)||score<7.5||yr<=1990)return false;if(/wwe|raw|smackdown/.test(t))return false;const gs=genres255(x,8).map(norm255);if(gs.length&&gs.every(g=>g==='drama'||g==='documentario'))return false;const key=key255(x);if(k.seen.has(key)||k.blocked.has(key)||k.shown.has(key)||(!allowWatch&&k.watch.has(key)))return false;if(fresh){const ds=String(x?.release_date||x?.first_air_date||'').slice(0,10);if(!ds||ds<shift255(-30)||ds>day255())return false}return true}
...
async function forYou255(force=false){
 ...
 const watch=[];for(const kind of ['movie','tv','anime']){const row=watchCandidates.find(x=>kind==='anime'?anime255(x):kind==='movie'?type255(x)==='movie':type255(x)==='tv'&&!anime255(x));if(row)watch.push(row)}
...
async function rowsDiscover255(tab,force=false){
 ...
 const k=await known255(false),ok=x=>eligible255(x,k);let task;
...
function sportCard255(e,p){const sports=new Map((p?.sports||[]).map(x=>[x.slug,x])),s=sports.get(e?.sport_slug)||{},match=e?.home_name||e?.away_name,live=String(e?.status||'').toLowerCase()==='live',finished=['finished','ended','final'].includes(String(e?.status||'').toLowerCase()),time=startMs255(e)?new Date(startMs255(e)).toLocaleString('pt-BR',{weekday:'short',day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}):'',score=e?.home_score!=null||e?.away_score!=null?`${esc255(e?.home_score??'–')} : ${esc255(e?.away_score??'–')}`:'×',key=String(e?.provider_event_id||e?.event_id||e?.id||''),watched=!!(e?.is_watched||e?.watched||e?.sport_watched_at);return `<article class="ct255-sport-card ${live?'live':''}"><div class="ct255-sport-top"><span>${esc255(s?.icon||'🏆')} ${esc255(e?.competition_name||s?.name||'Esporte')}</span><b>${live?'AO VIVO':finished?'ENCERRADO':esc255(new Date(startMs255(e)).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}))}</b></div>${match?`<div class="ct255-match"><div>${e?.home_logo?`<img src="${esc255(e.home_logo)}" alt="">`:''}<strong>${esc255(e?.home_name||'')}</strong></div><em>${score}</em><div>${e?.away_logo?`<img src="${esc255(e.away_logo)}" alt="">`:''}<strong>${esc255(e?.away_name||'')}</strong></div></div>`:`<h3>${esc255(e?.title||'Evento')}</h3>`}<small>${esc255(time)}${e?.venue?` · ${esc255(e.venue)}`:''}</small>${key?`<button type="button" class="ct255-watch ${watched?'on':''}" data-ct255-watch="${esc255(key)}" data-provider="${esc255(e?.provider||'')}" data-watched="${watched?'1':'0'}">${watched?'↶ Desmarcar assistido':'✓ Marcar como assistido'}</button>`:''}</article>`}
