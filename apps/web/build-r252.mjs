...
/* Historical composition can contain more than one call shape for the retired Sports RPC.
   Every executable rpc(...) call is redirected to the single canonical r252 payload helper. */
must(js,'cinetracker_sports_events_v0997','r248 sports-series RPC');
const retiredSportsCall=/rpc\(\s*['"]cinetracker_sports_events_v0997['"]\s*,\s*\{[^{}]*\}\s*\)/g;
js=js.replace(retiredSportsCall,"window.__ctR252SportsEvents()");
