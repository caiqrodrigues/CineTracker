...
function ct274AvailableText(x){const n=Math.max(0,Number(x?.available_episodes??x?.history_missing_episodes??(Number(x?.released_episodes||0)-Number(x?.watched_episodes||0))||0));return n===1?'1 episódio disponível para ver':n+' episódios disponíveis para ver'}
