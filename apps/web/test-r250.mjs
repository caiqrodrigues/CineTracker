...
const isR250Release=pkg.includes('"version": "1.0.41"');
if(isR250Release){
...
if(isR250Release){
 ...
 must(pkg,'build-r251-official.mjs','successor release official build');
 must(workflow,'Build inherited r250','successor workflow preserves inherited r250 build');
 must(workflow,'Static inherited r250','successor workflow preserves inherited r250 static regression');
}
...
console.log(`R250_STATIC_OK home=frontier discover=owned sports=canonical f1=single profile=merged scroll=local android=unchanged mode=${isR250Release?'release':'inherited'}`);
