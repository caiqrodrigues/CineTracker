...
const probe=`<script>setTimeout(async()=>{document.documentElement.dataset.ct336step='start';try{
 ...
 document.documentElement.dataset.ct336step='home';\n history.replaceState({},'','/home');
 ...
 await Promise.race([X.globalSearch('The Unraveling'),new Promise((_,rej)=>setTimeout(()=>rej(new Error('search timeout')),2500))]);
 ...
 document.documentElement.dataset.ct336step='done';document.documentElement.dataset.ct336done='1';
...
if(!/data-ct336done="1"/.test(out)){const m=out.match(/data-ct336probe="([^"]*)"/),st=out.match(/data-ct336step="([^"]*)"/);throw new Error('R336_BROWSER '+(m?.[1]||('probe did not finish step='+(st?.[1]||'none'))))}
