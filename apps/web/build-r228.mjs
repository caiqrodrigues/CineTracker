...
let [html,js,css,sw,patch,meta,guards,counts,ui]=await Promise.all([
 readFile(resolve(dist,'index.html'),'utf8'),readFile(resolve(dist,'app-v227.js'),'utf8'),readFile(resolve(dist,'app-v227.css'),'utf8'),readFile(resolve(dist,'service-worker.js'),'utf8'),readFile(resolve(root,'runtime-r228-v122-web-authority.js'),'utf8'),readFile(resolve(root,'runtime-r228b-v122-card-metadata.js'),'utf8'),readFile(resolve(root,'runtime-r228c-v122-authority-guards.js'),'utf8'),readFile(resolve(root,'runtime-r228d-v122-exact-count-authority.js'),'utf8'),readFile(resolve(root,'ui-refinement-v122.css'),'utf8')
]);
...
if(!ui.includes('pure UI refinement')||!ui.includes('--ct-ui-space-1:8px'))throw new Error('Web 1.0.22 UI refinement missing');
if(/#[0-9a-fA-F]{3,8}|rgba?\(|hsla?\(|\bcolor\s*:|\bbackground(?:-color)?\s*:|\bborder-color\s*:/.test(ui))throw new Error('Web 1.0.22 UI refinement must not define colors');
...
// 1.0.21 stays available for auth/session transport, but its DOM normalizers may no longer mutate Discover, Sports or Statistics.
...
await Promise.all([
 ...
 writeFile(resolve(dist,'release.json'),JSON.stringify({version:'1.0.22',revision:'r228-official-1.0.22',base:'r227-official-1.0.21',discover:'metadata+semantic-loading-r227-neutralized',sports:'global-final-two-actions-r227-neutralized',profile:'exact-renderable-watchlist-counts',ui:'pure-refinement-no-color-no-logic',generated_at:new Date().toISOString()},null,2),'utf8')
]);
await Promise.all([rm(resolve(dist,'app-v227.js'),{force:true}),rm(resolve(dist,'app-v227.css'),{force:true})]);
console.log('WEB_1_0_22_READY discover=metadata+series-settle sports=two-actions profile=exact-counts ui=pure-refinement r227-dom=neutralized');
