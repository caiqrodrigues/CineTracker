...
const HOME_CACHE261='ct-home-first-page-v261';
...
function clearNext261(r){r.next_season_number=null;r.next_episode_number=null;r.next_episode_title='';r.next_episode_air_date='';r.next_episode=null;r.next_episode_to_watch=null;r.next_unwatched_episode=null}

/* Persist one compact Home page beyond a single tab session. This is visual cache only;
   the canonical r5 payload still refreshes in the background. */
function persistHome261(){
  try{
    if(!homeCache?.series?.length)return;
    const pack=window.__ctR260Test?.compactHome260?.(homeCache)||{at:Date.now(),data:{...homeCache,series:(homeCache.series||[]).slice(0,90),movie_watchlist:(homeCache.movie_watchlist||[]).slice(0,18),history_episodes:[],history_movies:[]}};
    localStorage.setItem(HOME_CACHE261,JSON.stringify(pack));
  }catch{}
}
function restoreHome261(){
  try{
    const pack=JSON.parse(localStorage.getItem(HOME_CACHE261)||'null');
    if(!pack?.data?.series?.length)return null;
    if(Date.now()-n261(pack.at)>DAY261){localStorage.removeItem(HOME_CACHE261);return null}
    return pack.data;
  }catch{return null}
}
...
/* ---------------------------------------------------------------------
   ...
   Historical holes remain stored, but only already-aired episodes strictly AFTER the highest
   watched frontier count as current pending. Before the audit finishes, stale S01 backlog is
   neutralized so the user never sees S01E14 / Faltam 1495 flash back into the card. */
...
function provisionalWeekly261(row){
  if(!row||!weekly261(row)||row._ct261Weekly)return row;
  row._ct261Provisional=true;row.home_bucket='up_to_date';row.is_caught_up=true;row.history_missing_episodes=0;row.released_episodes=n261(row.watched_episodes);clearNext261(row);return row;
}
...
function applyWeekly261(row,seq){
  ...
  row._ct261Weekly=true;row._ct261Provisional=false;row._ct261CurrentMissing=missing;row._ct261CurrentNext=next||null;row.history_missing_episodes=missing;row.released_episodes=n261(row.watched_episodes)+missing;row.is_caught_up=!next;
  ...
  else{row.home_bucket=/ended|canceled|cancelled/.test(norm261(seq.detail?.status))?'completed':'up_to_date';clearNext261(row)}
...
const SPECIAL261={
 f1:{media_id:865,tmdb_id:-1321137963,title:'Formula 1',media_title:'Formula 1',release_year:1950,media_type:'tv',media_kind:'series'},
 superbowl:{media_id:837,tmdb_id:-1895884984,title:'NFL Super Bowls',media_title:'NFL Super Bowls',release_year:1967,media_type:'tv',media_kind:'series'}
};
...
function specialRow261(kind){return specialRows261().find(r=>specialKind261(r)===kind)||SPECIAL261[kind]||null}
function specialKindFromMedia261(target){const el=target?.closest?.('[data-media]');if(!el)return'';const raw=String(el.dataset.media||''),id=n261(raw.split(':').pop());if(id===SPECIAL261.f1.tmdb_id)return'f1';if(id===SPECIAL261.superbowl.tmdb_id)return'superbowl';const text=norm261(el.textContent||'');return specialKind261(text)}
...
function f1time261(x){const d=String(x?.air_date||x?.date||'').slice(0,10);if(!d)return'—';try{return new Intl.DateTimeFormat('pt-BR',{timeZone:'America/Sao_Paulo',day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'}).format(new Date(`${d}T${x?.time||'00:00:00Z'}`))}catch{return d}}
...
function superEpisodes261(season){const total=62,start=1967;return Array.from({length:total},(_,i)=>{const n=i+1,year=start+i;return{season_number:season,episode_number:n,name:season===2?`Super Bowl ${roman261(n)} Halftime Show`:`Super Bowl ${roman261(n)}`,year}})}
...
/* Discover rails are armed directly; touch remains native and r260 provides delegated mouse/pen drag. */
function armDiscover261(){for(const el of qa261('.ct259-discover-tabs,.ct259-discover-types,.ct259-media-rail')){el.dataset.ct260X='1';for(const c of ['flex','overflow-x-auto','scrollbar-thin','whitespace-nowrap','touch-pan-x','flex-nowrap'])el.classList.add(c)}}
const renderDiscover261Base=renderDiscover;
renderDiscover=async function(...args){const out=await renderDiscover261Base(...args);requestAnimationFrame(armDiscover261);setTimeout(armDiscover261,80);setTimeout(armDiscover261,500);return out};

/* Targeted Home wrappers: no global DOM observer. */
const paintHome261Base=paintHome;
paintHome=function(...args){for(const r of homeCache?.series||[])provisionalWeekly261(r);const out=paintHome261Base(...args);persistHome261();requestAnimationFrame(()=>{repairWeekly261();void patchSpecialHome261()});setTimeout(()=>{repairWeekly261();void patchSpecialHome261()},120);return out};
const renderHome261Base=renderHome;
renderHome=async function(seq){if(!(homeCache?.series?.length)){const cached=restoreHome261();if(cached)homeCache=cached}const out=await renderHome261Base(seq);if(seq===navSeq&&route()==='home'){setTimeout(()=>void auditWeekly261(),180);setTimeout(()=>void patchSpecialHome261(),40)}return out};

let specialReturn261='home';
document.addEventListener('click',e=>{
  const tagged=e.target?.closest?.('[data-ct261-special-series]'),kind=tagged?.dataset?.ct261SpecialSeries||specialKindFromMedia261(e.target);if(kind){const row=specialRow261(kind);if(row){specialReturn261=route()||'home';e.preventDefault();e.stopImmediatePropagation();void renderSpecialSeries261(kind,row)}return}
  const back=e.target?.closest?.('[data-ct261-special-back]');if(back){e.preventDefault();e.stopImmediatePropagation();if(specialReturn261==='home')void renderHome(++navSeq);else if(typeof go==='function')go('/'+specialReturn261);else void renderHome(++navSeq);return}
  const seasonBtn=e.target?.closest?.('[data-ct261-season]');if(seasonBtn){const root=q261('[data-ct261-special]');if(!root)return;e.preventDefault();e.stopImmediatePropagation();qa261('[data-ct261-season]',root).forEach(x=>x.classList.toggle('active',x===seasonBtn));const season=n261(seasonBtn.dataset.ct261Season),kind=root.dataset.ct261Special,row=specialRow261(kind);if(!row)return;void imported261(row).then(st=>kind==='f1'?renderF1Season261(st,season):renderSuperSeason261(st,season));return}
},true);

window.addEventListener('cinetracker:data-changed',()=>{stateCache261.clear();detailCache261.clear();seasonCache261.clear();importedState261.clear();f1SeasonCache261.clear();try{localStorage.removeItem(HOME_CACHE261)}catch{}});
window.__ctR261Test={specialKind261,specialKindFromMedia261,ep261,provisionalWeekly261,applyWeekly261,f1Episodes261,f1time261,roman261,watchedSet261,restoreHome261,armDiscover261};
})();
