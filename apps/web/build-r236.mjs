...
let [html,js,css,sw,patch,homeRepaint]=await Promise.all([
 ...
 readFile(resolve(root,'runtime-r236b-home-repaint.js'),'utf8')
]);
for(const marker of [
...
if(!homeRepaint.includes("window.__ctR236BHome='generic-home-hydration-repaint'"))throw new Error('Web 1.0.28 Home repaint marker missing');
for(const forbidden of ['Stuart','Lioness','WWE Raw'])if((patch+homeRepaint).includes(forbidden))throw new Error('Web 1.0.28 Home cannot hardcode '+forbidden);
