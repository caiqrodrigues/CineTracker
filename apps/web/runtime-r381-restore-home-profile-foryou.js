...
function profileHtml381(d){
 ...
 return '<section class="panel"><div class="panel-head"><h2>Estatísticas</h2><small>sincronizadas</small></div><div class="stats">'+
  ...
  (rows(d?.favorite_actors).slice(0,10).map(a=>{const p=a.profile_path?(typeof img==='function'?img(a.profile_path,'w185'):a.profile_path):'';return '<article class="card"><button type="button" data-person="'+Number(a.tmdb_person_id||0)+'"><div class="poster"'+(p?' style="background-image:url(\''+esc381(p)+'\')"':'')+'></div><div class="card-body"><b>'+esc381(a.actor_name||'Ator')+'</b><small>Ator favorito</small></div></button></article>'}).join('')||'<div class="empty">Nenhum ator favorito.</div>')+
