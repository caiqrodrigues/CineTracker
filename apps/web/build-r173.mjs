...
js=js.replace("const REVISION='r172-home-episode-streaming-clean';","const REVISION='r173-detail-left-window';");
js=js.replace('\nboot();','\n'+patch+'\nboot();');
css+='\n'+style+'\n';
html=html.replaceAll('r172-home-episode-streaming-clean','r173-detail-left-window').replaceAll('app-v172.js','app-v173.js').replaceAll('app-v172.css','app-v173.css');
sw=sw.replaceAll('r172-home-episode-streaming-clean','r173-detail-left-window').replaceAll('app-v172.js','app-v173.js').replaceAll('app-v172.css','app-v173.css');
await Promise.all([
  writeFile(resolve(dist,'index.html'),html,'utf8'),
  writeFile(resolve(dist,'app-v173.js'),js,'utf8'),
  writeFile(resolve(dist,'app-v173.css'),css,'utf8'),
  writeFile(resolve(dist,'service-worker.js'),sw,'utf8'),
  writeFile(resolve(dist,'release.json'),JSON.stringify({version:'0.99.7',revision:'r173-detail-left-window',runtime:'single-clean-runtime',generated_at:new Date().toISOString()}),'utf8')
]);
await Promise.all([rm(resolve(dist,'app-v172.js'),{force:true}),rm(resolve(dist,'app-v172.css'),{force:true})]);
console.log('WEB_R173_READY detail=left-windowed-hero poster+title+meta+synopsis+actions');
