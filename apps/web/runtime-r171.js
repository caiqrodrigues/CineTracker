...
async function ct171TopRows(provider){
  ...
  try{const c=JSON.parse(localStorage.getItem(key)||'null');if(c?.data?.movies&&c?.data?.series&&Date.now()-Number(c.at||0)<900000){try{sessionStorage.setItem(key,JSON.stringify(c.data))}catch{}return c.data}}catch{}
  ...
  const [m,t]=await Promise.all([safeTmdb('/discover/movie',common),safeTmdb('/discover/tv',common)]),data={movies:(m.results||[]).filter(x=>x.id&&x.poster_path).slice(0,10).map(x=>({...x,media_type:'movie'})),series:(t.results||[]).filter(x=>x.id&&x.poster_path).slice(0,10).map(x=>({...x,media_type:'tv'}))};try{sessionStorage.setItem(key,JSON.stringify(data));localStorage.setItem(key,JSON.stringify({at:Date.now(),data}))}catch{}return data;
}
