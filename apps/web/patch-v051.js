...
const st=document.createElement('style');st.textContent='.ct-version-footer{display:none!important}';document.head.appendChild(st);
...
function versionFooter(){if(typeof view==='undefined'||view!=='settings')return;const host=$('.content')||$('#app');if(!host)return;for(const x of $$('*',host)){if(x.children.length===0&&/CineTracker Web\s*[•·-]?\s*(?:vers[aã]o|build)\s+0\./i.test((x.textContent||'').trim())&&!x.id?.includes('ct51w'))x.remove()}let f=$('#ct51w-version',host);if(!f){f=document.createElement('div');f.id='ct51w-version';f.style.cssText='margin:28px 0 8px;text-align:center;color:#71808b;font-size:11px';host.appendChild(f)}f.textContent=`CineTracker Web • versão ${VERSION}`}
