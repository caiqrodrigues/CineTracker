...
try{for(const width of [420,1200]){const out=execFileSync(bin,['--headless','--no-sandbox','--disable-gpu','--disable-background-networking','--disable-component-update','--disable-sync','--no-first-run','--no-default-browser-check',`--user-data-dir=${resolve(dir,'p'+width)}`,`--window-size=${width},900`,'--virtual-time-budget=3600','--dump-dom','file://'+page],{encoding:'utf8',timeout:30000,maxBuffer:16*1024*1024,stdio:['ignore','pipe','pipe']});const errors=(out.match(/data-errors="([^"]*)"/)||[])[1]||'';if(errors)throw new Error('R272 browser '+width+' errors '+errors);for(const x of['data-done="1"','data-pending-series-first="true"','data-pending-movies-first="true"','data-pending-loading="true"','data-pending-no-false-empty="true"','data-history-series-first="true"','data-history-movies-first="true"','data-history-series="true"','data-history-movies="true"','data-history-counts="true"','data-watch-parent="true"','data-watch-position="absolute"','data-watch-inside="true"','data-clicks="100"','data-timer="fired"'])if(!out.includes(x))throw new Error('R272 browser '+width+' missing '+x)}console.log('R272_BROWSER_OK exact-shipped-bundle history-first r5-data direct-watch responsive mobile+desktop')}finally{await rm(dir,{recursive:true,force:true})}
