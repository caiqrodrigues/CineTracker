...
const [binding,following,discoverFinal]=await Promise.all([
 ...
 readFile(resolve(root,'runtime-r248-discover-final.js'),'utf8')
]);
...
if(!discoverFinal.includes("window.__ctR248DiscoverFinal='content-only-tabs-stable-cards-personal-rules'"))throw new Error('r248 final Discover authority missing');
if(!js.includes('\nboot();'))throw new Error('r248 final boot insertion point missing');
js=js.replace('\nboot();','\n'+binding+'\n'+following+'\n'+discoverFinal+'\nboot();');
await writeFile(resolve(dist,'app-v248.js'),js,'utf8');
console.log('WEB_1_0_39_OFFICIAL r248 binding+following+discover-final=ready');
