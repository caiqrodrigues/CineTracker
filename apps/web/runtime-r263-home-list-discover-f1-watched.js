...
function f1Rows263(payload,now=Date.now()){const events=Array.isArray(payload?.events)?payload.events:[],hist=Array.isArray(payload?.watch_history)?payload.watch_history:[],map=new Map();for(const x of [...events,...hist]){if(String(x?.sport_slug||'')!=='formula_1')continue;const k=sportEventKey263(x);if(!k||k===':')continue;map.set(k,{...(map.get(k)||{}),...x})}const from=now-21*DAY263,to=now+35*DAY263;return [...map.values()].filter(x=>{const t=new Date(x?.starts_at||0).getTime();return Number.isFinite(t)&&t>=from&&t<=to}).sort((a,b)=>new Date(b?.starts_at||0)-new Date(a?.starts_at||0)).slice(0,16)}
...
function paintF1WatchButton263(btn,watched){if(!btn)return;btn.dataset.ct263Watched=watched?'1':'0';btn.classList.toggle('on',watched);btn.textContent=watched?'↶ Desmarcar assistido':'✓ Marcar como assistido';const card=btn.closest('.ct263-f1-event');if(card)card.classList.toggle('watched',watched)}
function updateF1Payload263(provider,id,watched,at){const p=f1Watch263.payload||(f1Watch263.payload={}),key=String(provider||'')+':'+String(id||''),events=Array.isArray(p.events)?p.events:(p.events=[]),hist=Array.isArray(p.watch_history)?p.watch_history:(p.watch_history=[]);let base=null,was=false;for(const x of [...events,...hist])if(sportEventKey263(x)===key){base=base||x;was=was||!!(x?.is_watched||x?.watched||x?.sport_watched_at||x?.watched_at)}for(const x of events)if(sportEventKey263(x)===key){x.is_watched=watched;x.watched=watched;x.sport_watched_at=watched?at:null;x.watched_at=watched?at:null}p.watch_history=hist.filter(x=>sportEventKey263(x)!==key);if(watched&&base)p.watch_history.unshift({...base,is_watched:true,watched:true,sport_watched_at:at,watched_at:at});f1Watch263.at=Date.now();return{delta:watched===was?0:(watched?1:-1)}}
async function f1WatchRpc263(args){const bridge=window.__ctR368SportsTestBridge;if(bridge?.rpc)return bridge.rpc('f1',args);return rpc('cinetracker_sport_mark_watched_v1',args)}
async function toggleF1Watch263(btn){if(!btn||btn.dataset.ct368SportBusy==='1')return false;const provider=btn.dataset.ct263Provider,id=btn.dataset.ct263F1Watch,watched=btn.dataset.ct263Watched==='1';if(!provider||!id)return false;const next=!watched,at=new Date().toISOString(),snapshot=f1Watch263.payload?JSON.parse(JSON.stringify(f1Watch263.payload)):null;btn.dataset.ct368SportBusy='1';paintF1WatchButton263(btn,next);btn.disabled=true;try{const local=updateF1Payload263(provider,id,next,at);await f1WatchRpc263({p_provider:provider,p_provider_event_id:id,p_watched:next,p_watched_at:at,p_duration_minutes:null});window.dispatchEvent(new CustomEvent('cinetracker:sports-watched-changed',{detail:{source:'f1',provider,id,watched:next,delta:local.delta}}));return true}catch(e){f1Watch263.payload=snapshot;paintF1WatchButton263(btn,watched);return false}finally{delete btn.dataset.ct368SportBusy;if(btn.isConnected)btn.disabled=false}}
...
window.__ctR263Test={shift263,strictEligible263,browseEligible263,pickProviders263,f1Rows263,sportEventKey263,restoreHomeList263,toggleF1Watch263,updateF1Payload263,paintF1WatchButton263};
