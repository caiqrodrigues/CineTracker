...
for(const m of ["window.__ctR212='v107-direct-rewatch-authority'",'ct212Bound','cinetracker_mark_watch_v0994','stopImmediatePropagation'])if(!rewatch.includes(m))throw new Error('Web 1.0.7 replay authority missing '+m);
...
// The 1.0.4 base contains a legacy episode RPC that never existed in production. Disable it; r212 owns visible replay controls.
...
// r212 MUST register first so its document capture listener beats legacy/delegated replay handlers.
js=js.replace('\nboot();','\n'+rewatch+'\n'+patch+'\nboot();');
