...
let [html,js,css,sw,patch,rewatch,sports]=await Promise.all([
 readFile(resolve(dist,'index.html'),'utf8'),readFile(resolve(dist,'app-v208.js'),'utf8'),readFile(resolve(dist,'app-v208.css'),'utf8'),readFile(resolve(dist,'service-worker.js'),'utf8'),readFile(resolve(root,'runtime-r211-v107-behavior.js'),'utf8'),readFile(resolve(root,'runtime-r212-v107-rewatch-authority.js'),'utf8'),readFile(resolve(root,'runtime-r213-v107-sports-summary.js'),'utf8')
]);
...
for(const m of ["window.__ctR212='v107-direct-rewatch-authority'",'__CT107_RPC__','__CT107_ENSURE__','cinetracker_mark_watch_v0994','stopImmediatePropagation'])if(!rewatch.includes(m))throw new Error('Web 1.0.7 replay authority missing '+m);
for(const m of ["window.__ctR213='v107-sports-summary-authority'",'[data-sports]','.sports-summary'])if(!sports.includes(m))throw new Error('Web 1.0.7 Sports cleanup missing '+m);
...
rewatch=rewatch.replace('__CT107_RPC__','((name,args)=>rpc(name,args))').replace('__CT107_ENSURE__','((type,id)=>ensureMedia(type,id))');
if(rewatch.includes('__CT107_RPC__')||rewatch.includes('__CT107_ENSURE__'))throw new Error('Web 1.0.7 replay dependency binding failed');
...
console.log('WEB_1_0_7_READY revision=r211-official-1.0.7 rewatch=r212 sports=r213');