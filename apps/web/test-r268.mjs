...
has(js,"window.__ctR268='home-history-authority+history-first+inline-right-watch'",'r268 marker');
has(js,"window.__ctR268Home='canonical-history+history-first+same-row-right-watch'",'Home marker');
...
has(js,"function ct268IsHistorySection(sec){const title=ct268HistoryTitle(sec);return title==='historico recente'||title==='filmes vistos'}",'both Home history sections identified');
has(js,'function ct268PinHistoryFirst(sec)','history-first reconciler');
has(js,'parent.insertBefore(sec,firstNonHistory);','history section moved before normal Home sections');
has(js,"sec.dataset.ct268HistoryFirst='1'",'history-first marker');
...
const meta=JSON.parse(release);ok(meta.version==='1.0.59'&&meta.revision==='r268-official-1.0.59','release identity');ok(meta.home_history_authority===true&&meta.home_history_first===true&&meta.home_watch_inline_right===true,'release Home fix flags');ok(meta.discover==='r267-preserved'&&meta.detail==='r267-preserved'&&meta.sports==='r267-preserved','frozen Web surfaces preserved');ok(meta.android==='1.0.20/10062','Android baseline preserved');
...
console.log('R268_STATIC_OK canonical-history history-first inline-right-watch frozen-surfaces-preserved');
