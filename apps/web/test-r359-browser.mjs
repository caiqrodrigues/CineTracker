...
const probe=`<script>setTimeout(async()=>{try{
 ...
 ok(window.__ctR359&&window.__ctR352&&window.__ctR336&&window.__ctR309Test,'r359 lineage missing '+window.__ctDiagErrors.join(' || '));
 ...
 window.__ctR359.setTestBridge({
  watchlist:async(type,id)=>{calls.watchlist.push(type+':'+id);await sleep(15);return true},
  seen:async(type,id)=>{calls.seen.push(type+':'+id);await sleep(15);return true}
 });
 ok(window.__ctR336.paintForYou(),'Pra Você paint failed');window.__ctR348?.fixAll?.();window.__ctR349?.compact?.();await sleep(70);

 const root=document.querySelector('[data-ct336-foryou]');ok(root,'Pra Você root missing');
 const slotNodes=()=>Object.fromEntries([...root.querySelectorAll('[data-ct336-slot]')].map(s=>[s.dataset.ct336Slot,s]));
 const keys=()=>Object.fromEntries([...root.querySelectorAll('[data-ct336-slot]')].map(s=>[s.dataset.ct336Slot,s.querySelector('[data-ct288-card]')?.dataset?.ct288Card||'']));
 ...
 const sameOtherNodes=(beforeNodes,name)=>{
  ok(document.querySelector('[data-ct336-foryou]')===root,'Pra Você root was globally repainted');
  const afterNodes=slotNodes();
  for(const k of Object.keys(beforeNodes))if(k!==name)ok(beforeNodes[k]===afterNodes[k],k+' DOM node was globally repainted');
 };
 const sabotage=btn=>btn.addEventListener('click',e=>{e.stopImmediatePropagation();e.stopPropagation()},true);

 let before=keys(),nodes=slotNodes();
 ...
 let after=keys();changedOnly(before,after,'fresh:movie');sameOtherNodes(nodes,'fresh:movie');await sleep(30);
 ok(calls.watchlist.length===1&&calls.watchlist[0]==='movie:401','Watchlist backend wrong '+calls.watchlist);

 before=keys();nodes=slotNodes();
 ...
 after=keys();changedOnly(before,after,'watch:series');sameOtherNodes(nodes,'watch:series');await sleep(30);
 ok(calls.seen.length===1&&calls.seen[0]==='tv:201','Seen backend wrong '+calls.seen);
 ...
 after=keys();changedOnly(before,after,'fresh:anime');sameOtherNodes(nodes,'fresh:anime');
 ...
 /* Cached Home payload: title/rating/date are complete before paint and no hydrate is needed. */
 history.replaceState({},'','/');
 const homePayload={
  series:[{media_id:9001,tmdb_id:777,media_type:'tv',title:'Série Cache',poster_path:null,home_bucket:'continue',watched_episodes:3,released_episodes:5,total_episodes:5,
   next_season_number:1,next_episode_number:4,next_episode_title:'O Episódio Pronto',next_episode_rating:8.6,next_episode_air_date:'2026-09-23',available_episodes:2,__ct359_episode_cache:true}],
  movie_watchlist:[],history_episodes:[],history_movies:[],__ct_home_authority:'home-v359-episode-cache-first-paint'
 };
 window.__ctR359.setTestBridge({homePayload:async()=>structuredClone(homePayload),homeHistory:async()=>({history_episodes:[],history_movies:[]})});
 const fetched=await window.__ctR359.fetchHome();
 ok(window.__ctR359.payloadReady(fetched),'cached Home payload considered incomplete');
 ok(await window.__ctR359.prepare(fetched,undefined),'cached Home prepare failed');
 ok(fetched.__ct359PreparedFromCache===true,'Home did not short-circuit to episode cache');
 ok(document.documentElement.dataset.ct359HomeSource==='episode-cache','Home source not cache');

 document.querySelector('#app').innerHTML='<main><div data-home><article data-ct274-episode-card data-ct274-fallback-title="O Episódio Pronto" data-ct274-fallback-rating="8.6" data-ct274-fallback-date="2026-09-23"><div class="ct274-meta">S01E04 • Ep: O Episódio Pronto • ⭐ 8.6 • 23/09/2026</div></article></div></main>';
 ok(await window.__ctR359.hydrateDom(),'cached Home hydrate short-circuit failed');
 ok(document.documentElement.dataset.ct359HomeHydrate==='skipped-complete-cache','cached Home tried late episode hydration');
 ok(document.querySelector('.ct274-meta').textContent.includes('O Episódio Pronto'),'Home metadata changed after cache-ready');
...
console.log('R359_BROWSER_OK physical direct clicks change only target slot + cached Home episode metadata skips late hydration');
