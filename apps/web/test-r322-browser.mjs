...
const probe=`<script>setTimeout(async()=>{try{
 const ok=(v,m)=>{if(!v)throw new Error(m)},T=window.__ctR321Test,R=window.__ctR321;ok(T&&R,'r321 bridge unavailable');ok(window.__ctR322==='discover-indexed-user-filter+fast-top10','r322 marker unavailable');
 ...
 document.documentElement.dataset.ct322done='1';
}catch(e){document.documentElement.dataset.ct322probe='fail:'+String(e?.stack||e)}},4400)</script>`;
...
if(!/data-ct322done="1"/.test(out)){const m=out.match(/data-ct322probe="([^"]*)"/);throw new Error('R322_BROWSER '+(m?.[1]||'probe did not finish'))}
console.log('R322_BROWSER_OK pre-render filter + visible load');
