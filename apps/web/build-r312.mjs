...
"  if(!ctSession?.refresh_token)throw new Error('Sua sessão expirou. Entre novamente.');",
"  const fresh=await authRequest('token?grant_type=refresh_token',{refresh_token:ctSession.refresh_token});",
"  saveSession(fresh);return ctSession;",
" })().finally(()=>{ct312RefreshPromise=null});",
" return ct312RefreshPromise;",
"}",
"async function ct312EnsureFresh(){",
" const exp=Number(ctSession?.expires_at||0)*1000;",
" if(ctSession?.refresh_token&&exp&&exp<=Date.now()+30000)await ct312RefreshAccess();",
"}",
"async function sbApi(path, options = {}, allowRetry = true) {",
"    if (!ctSession?.access_token)",
"        throw new Error('Faça login para continuar');",
"    await ct312EnsureFresh();",
"    const headers = { ...(options.headers || {}), ...authHeaders(), 'Content-Type': 'application/json', Prefer: 'return=representation' };",
"    const r = await fetch(SUPABASE_URL+'/rest/v1/'+path, { ...options, headers });",
"    const text = await r.text();",
"    let d=null;try{d=text?JSON.parse(text):null}catch{d=text?{message:text}:null}",
"    if (!r.ok && allowRetry && ct312JwtExpired(r.status,d)) {",
"        await ct312RefreshAccess();",
"        return sbApi(path, options, false);",
"    }",
"    if (!r.ok)",
"        throw new Error(d?.message || d?.hint || d?.details || ('Banco '+r.status));",
"    return d;",
"}"
].join('\n');
...
"    if (!ctSession?.access_token)",
"        throw new Error('Faça login para pesquisar');",
"    await ct312EnsureFresh();",
...
"    if (!r.ok && allowRetry && ct312JwtExpired(r.status,d)) {",
"        await ct312RefreshAccess();",
"        return tmdbFetch(params, false);",
"    }",
"    if (!r.ok)",
"        throw new Error(d.error || d.message || ('TMDB '+r.status));",
"    return d;",
"}"
].join('\n');
