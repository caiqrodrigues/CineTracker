...
let [html,js,css,sw,patch]=await Promise.all([
 ...
 readFile(resolve(dist,'app-v205.js'),'utf8'),
 readFile(resolve(dist,'app-v205.css'),'utf8'),
 readFile(resolve(root,'service-worker.js'),'utf8'),
 readFile(resolve(root,'runtime-r208-v104-core.js'),'utf8')
]);
...
if(!swVersion.test(sw))throw new Error('Canonical service worker VERSION declaration missing');
sw=sw.replace(swVersion,"const VERSION='ct-web-1.0.4-r208';");
await Promise.all([
 writeFile(resolve(dist,'index.html'),html,'utf8'),
 writeFile(resolve(dist,'app-v208.js'),js,'utf8'),
 writeFile(resolve(dist,'app-v208.css'),css,'utf8'),
...
console.log('WEB_1_0_4_READY runtime=internal-before-boot revision=r208-official-1.0.4 sw=canonical-1.0.4');
