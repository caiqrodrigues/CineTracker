...
function norm13(s){return String(s||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim()}
...
function activeShow13(x){return x.type==='show'&&['following','watching','for_later'].includes(String(x.list_status||''))}
...
function prepareImport13(inputLibrary,inputWatches){
  const original=(inputLibrary||[]).map((x,i)=>({...x,import_key:String(i+1)}));
  ...
  function synthesize(w,t){
    ...
    const candidate={type,title:w.title||w.show_title||w.movie_title||'Sem título',original_title:w.original_title||'',year:w.year||w.release_year||'',tmdb_id:w.tmdb_id||'',tvdb_id:w.tvdb_id||'',list_status:type==='show'?'watching':'',history_only:true};
  ...
  for(let i=0;i<watches.length;i++){
    ...
    if(!m){const a=byTitle.get(`${t}:${norm13(w.title)}`)||[];m=a.length===1?a[0]:a.find(x=>Number(x.year)<=Number(String(w.first_watched_at||w.last_watched_at||'').slice(0,4)))||a[0]}
    if(!m&&(w.title||w.show_title||w.movie_title||w.tmdb_id||w.tvdb_id))m=synthesize(w,t);
    if(!m)continue;
    mapped.push({...w,source_history_id:i+1,media_tmdb_id:stableId13(m),media_type:m.type==='movie'?'movie':'tv'});
  }
  const watchedShowIds=new Set(mapped.filter(x=>x.type==='episode').map(x=>Number(x.media_tmdb_id)));
  for(const x of library){
    if(x.type!=='show')continue;
    const watched=watchedShowIds.has(Number(stableId13(x)));
    if(x.history_only){x.list_status=watched?'watching':x.list_status;continue}
    if(!activeShow13(x))continue;
    if(String(x.list_status)==='for_later'){x.ct13_derived_state='watchlist_explicit';continue}
    if(watched){x.list_status='watching';x.ct13_derived_state='started'}
    else{x.list_status='for_later';x.ct13_derived_state='not_started_watchlist'}
  }
  const movies=original.filter(x=>x.type==='movie').length;
  const series=original.filter(x=>x.type==='show').length;
  const watchedMovies=mapped.filter(x=>x.type==='movie').length;
  const episodes=mapped.filter(x=>x.type==='episode').length;
  const watchMovies=original.filter(x=>x.type==='movie'&&x.list_status==='for_later').length;
  const watchSeries=library.filter(x=>!x.history_only&&x.type==='show'&&x.list_status==='for_later').length;
  const followed=library.filter(x=>!x.history_only&&x.type==='show'&&activeShow13(x)&&x.list_status!=='for_later'&&watchedShowIds.has(Number(stableId13(x)))).length;
  return {library,watches:mapped,summary:{library_items:original.length,library_movies:movies,library_series:series,import_media_items:library.length,watch_events:mapped.length,raw_watch_events:watches.length,watched_movie_events:watchedMovies,watched_episode_events:episodes,watchlist_movies:watchMovies,watchlist_series:watchSeries,followed_series:followed,history_only_media:historyOnly.length,unmatched_watch_events:Math.max(0,watches.length-mapped.length)}};
}
...
function preview13(data,name){
  ...
  const mismatch=s.unmatched_watch_events?`<div class="ct10-safe" style="border-color:#7d5530;background:#28180b">Atenção: ${s.unmatched_watch_events} evento(s) do histórico não possuem identificação suficiente para importação.</div>`:`<div class="ct10-safe">Histórico reconhecido integralmente: ${s.watch_events}/${s.raw_watch_events} eventos. Itens vistos que não estão mais na biblioteca atual também serão preservados.</div>`;
  o.innerHTML=`<div class="ct10-preview-card"><h2>Prévia da importação</h2><p class="ct10-muted">Nenhum dado foi alterado. Confira os números antes de confirmar.</p>${mismatch}<div class="ct10-preview-grid"><div class="ct10-metric"><b>${s.library_items}</b>Itens da biblioteca</div><div class="ct10-metric"><b>${s.library_movies}</b>Filmes na biblioteca</div><div class="ct10-metric"><b>${s.library_series}</b>Séries na biblioteca</div><div class="ct10-metric"><b>${s.watched_movie_events}</b>Filmes assistidos</div><div class="ct10-metric"><b>${s.watched_episode_events}</b>Episódios assistidos</div><div class="ct10-metric"><b>${s.watchlist_movies+s.watchlist_series}</b>Watchlist</div><div class="ct10-metric"><b>${s.watchlist_movies}</b>Watchlist filmes</div><div class="ct10-metric"><b>${s.watchlist_series}</b>Watchlist séries (não iniciadas)</div><div class="ct10-metric"><b>${s.followed_series}</b>Séries com histórico</div></div><p class="ct10-muted" style="margin-top:10px">${s.history_only_media} mídia(s) do histórico não estão mais na biblioteca atual e serão recriadas somente para preservar o histórico.</p><div id="ct10-progress-wrap" hidden><div class="ct10-progress"><span></span></div><div class="ct10-progress-label">Preparando…</div></div><div class="ct10-actions"><button class="ct10-btn" data-close13>Cancelar</button><button class="ct10-btn" data-confirm13 ${s.unmatched_watch_events?'disabled':''}>Confirmar importação</button></div></div>`;
...
async function run13(){
  ...
  try{
    ...
    const begin=await edge13({action:'begin',filename:p.name,file_type:p.name.toLowerCase().endsWith('.json')?'json':'zip',total_items:p.summary.import_media_items+p.summary.watch_events}),id=begin.import_id,batch=150,total=Math.max(1,p.library.length+p.watches.length);let done=0;
    for(let i=0;i<p.library.length;i+=batch){const rows=p.library.slice(i,i+batch);await edge13({action:'library_batch',import_id:id,rows,cursor:done,progress:Math.round(done/total*100)});done+=rows.length;progress13(5+Math.round(done/total*85),`Importando biblioteca e referências históricas: ${Math.round((i+rows.length)/p.library.length*100)}%`);await sleep13(0)}
    for(let i=0;i<p.watches.length;i+=batch){const rows=p.watches.slice(i,i+batch);await edge13({action:'watches_batch',import_id:id,rows,cursor:done,progress:Math.round(done/total*100)});done+=rows.length;progress13(5+Math.round(done/total*85),`Importando histórico completo: ${Math.round((i+rows.length)/p.watches.length*100)}%`);await sleep13(0)}
    progress13(95,'Atualizando o CineTracker…');await edge13({action:'finish',import_id:id,summary:p.summary});progress13(100,'Importação concluída.');
...
async function readImportFiles13(files){
  ...
  if(!data.watches.length)throw new Error('watches.csv/JSON não contém eventos válidos.');
  preview13(data,name);
}
