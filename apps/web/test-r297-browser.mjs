...
const probe=`<script>setTimeout(()=>{const a=document.querySelector('#app');document.documentElement.dataset.ct297=String(window.__ctR297||'');document.documentElement.dataset.ct297scope=String(window.__ctR295BrowseSelfScopeFix||'');document.documentElement.dataset.ct297app=String((a?.innerHTML||'').length);document.documentElement.dataset.ct297errors=String((window.__ct297Errors||[]).join(' | '));},3500);</script>`;
...
try{
  ...
  if(!tag.includes('data-ct297="r295-scope-post-boot-hotfix-full-bundle"'))throw new Error('R297_FULL_BUNDLE marker missing '+tag);
  if(!tag.includes('data-ct297scope="decorate-browse-host-self-and-descendants"'))throw new Error('R297_FULL_BUNDLE r295 scope did not install after boot '+tag);
  ...
  console.log('R297_FULL_BUNDLE_BOOT_OK exact app-v297.js booted, rendered #app and installed r295 scope after authority');
