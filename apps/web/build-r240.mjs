...
for(const marker of ["window.__ctR240='sports-four-data-authority'","window.__ctR240Sports='next-today-only+previous-last-3-days+favorites-only+watched'","sportsTabs=function()","sportsPayload=async function()","sportsFiltered=function(rows)"]){if(!patch.includes(marker))throw new Error('r240 missing '+marker)}
...
await Promise.all([
 ...
 writeFile(resolve(dist,'release.json'),JSON.stringify({version:'1.0.32',revision:'r240-official-1.0.32',base:'r239-official-1.0.31',sports_tabs:['Próximos','Anteriores','Favoritos','Assistidos'],sports_next:'today-future-only',sports_previous:'previous-three-calendar-days-only',sports_favorites:'favorite-only',sports_watched:'preserve-canonical-watched-feed',android:'unchanged-1.0.20',generated_at:new Date().toISOString()},null,2),'utf8')
]);
await Promise.all([rm(resolve(dist,'app-v239.js'),{force:true}),rm(resolve(dist,'app-v239.css'),{force:true})]);
console.log('WEB_1_0_32_READY r240 sports=four-data-tabs');