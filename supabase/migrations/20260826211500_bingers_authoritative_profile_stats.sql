-- CineTracker: use verified Bingers profile totals when the import provides them.
-- Generic fallback behavior remains unchanged for profiles without authoritative totals.
...
ep as (
  select
    ...
    (select count(*) from ep_series)::bigint as series_watched
),
wh_mv as (
  select
    wh.media_id,
...
base as (
  select ep.episodes_watched, mv.movies_watched, ep.series_watched,
