-- Reconciles the first r314 F1 writer already applied to production on 2026-09-18.
-- This SECURITY DEFINER helper is intentionally removed by 20260918182743.
...
begin
  ...
  v_duration := case v_kind when 'race' then interval '2 hours' else interval '1 hour' end;

  insert into public.sport_events(
    sport_slug,provider,provider_event_id,title,starts_at,ends_at,status,season,round,venue,participants,raw,last_synced_at,updated_at
  ) values (
    'formula_1','jolpica',v_event_key,coalesce(v_title,'Fórmula 1'),p_starts_at,p_starts_at+v_duration,
    case when p_starts_at <= now() then 'finished' else 'scheduled' end,v_season,v_round,
    nullif(trim(coalesce(p_metadata->>'venue','')),''),'[]'::jsonb,
    jsonb_build_object('ct_source','r314','kind',v_kind)||coalesce(p_metadata,'{}'::jsonb),now(),now()
  )
  on conflict(provider,provider_event_id) do update
    set title=excluded.title,starts_at=excluded.starts_at,ends_at=excluded.ends_at,status=excluded.status,
        season=excluded.season,round=excluded.round,venue=coalesce(excluded.venue,public.sport_events.venue),
        raw=coalesce(public.sport_events.raw,'{}'::jsonb)||excluded.raw,last_synced_at=now(),updated_at=now()
