...
const jsonHeaders={'content-type':'application/json','cache-control':'private, max-age=120','access-control-allow-origin':'*'};
...
Deno.serve(async(req:Request)=>{
 ...
 try{
  const body=await req.json().catch(()=>({}));const action=String(body?.action||'overview');const season=String(body?.season||new Date().getUTCFullYear());
  ...
  const common={season:Number(season),generated_at:new Date().toISOString(),races:all,next,last};
  if(action==='calendar')return new Response(JSON.stringify(common),{headers:jsonHeaders});
  ...
  return new Response(JSON.stringify({...common,drivers,constructors,results,qualifying,sprint,pitstops,laps}),{headers:jsonHeaders});
