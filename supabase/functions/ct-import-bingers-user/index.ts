...
Deno.serve(async(req)=>{try{
  ...
  if(action==='begin'){
    ...
    const r=await rest('imports',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({
      ...
      summary:{source:'Bingers',strategy:'hotfix13_plays_semantics',phase:'begin',preserves_manual:true,ratings_ignored:true,lists_ignored:true}
    })});
  ...
  if(action==='library_batch'){
    ...
    await rest(`imports?id=eq.${importId}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({processing_cursor:cursor,summary:{source:'Bingers',strategy:'hotfix13_plays_semantics',phase:'library',progress:Number(b.progress||0),preserves_manual:true}})});
  ...
  if(action==='watches_batch'){
    ...
    await insert('media_overrides',ovs,'profile_id,media_id,state','ignore');
    const cursor=Number(b.cursor||0)+rows.length;
    await rest(`imports?id=eq.${importId}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({processing_cursor:cursor,summary:{source:'Bingers',strategy:'hotfix13_plays_semantics',phase:'history',progress:Number(b.progress||0),preserves_manual:true}})});
    return json({ok:true,count:rows.length,cursor});
  }
  ...
  if(action==='finish'){
    ...
    await rest(`imports?id=eq.${importId}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({status:'completed',matched_items:Number(s.library_items||0),unmatched_items:Number(s.unmatched_watch_events||0),processing_cursor:Number(s.import_media_items||s.library_items||0)+Number(s.raw_watch_records||0),summary:{source:'Bingers',strategy:'hotfix13_plays_semantics',preserves_manual:true,ratings_ignored:true,lists_ignored:true,...s},completed_at:new Date().toISOString(),updated_at:new Date().toISOString()})});
