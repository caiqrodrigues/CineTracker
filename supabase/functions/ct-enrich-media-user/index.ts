...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const score=(x:any)=>
      (x.is_in_progress?1000:0)+(x.is_up_to_date?700:0)+(Number(x.watched_episodes||0)>0?500:0)+(x.is_watchlist?250:0)+
      (Number(x.tmdb_id||0)<=0?180:0)+(!x.poster_path?120:0)+(x.media_type==='tv'&&!x.raw_tmdb?.last_episode_to_air?80:0)+
      Math.min(50,Math.max(0,(Date.parse(x.last_watched_at||'')||0)/1e12));
    const ids=rowsDash.filter(needs).sort((a:any,b:any)=>score(b)-score(a)).slice(0,limit).map((x:any)=>Number(x.media_id)).filter(Boolean);
    if(!ids.length) return json({processed:0,ok:0,fail:0,resolved_surrogates:0,official_refreshed:0,remaining:0,priority:'current-user'});
      ...
      await Promise.all(rows.slice(i,i+6).map(async(m:any)=>{
        try{
          ...
          if(tmdbId>0){
            ...
            const qp=new URLSearchParams({query:String(m.title||'').replace(/\s*\((?:19|20)\d{2}\)\s*$/,''),language:'pt-BR',include_adult:'false'});
            ...
            const candidates=(sd.results||[])
              .map((x:any)=>({x,n:norm(x.title||x.name),base:baseTitle(x.title||x.name),y:yearOf(x)}))
              .filter((z:any)=>(z.n===want||z.base===baseWant)&&(!sourceYear||!z.y||Math.abs(z.y-sourceYear)<=1));
          ...
          const raw={...(detail||{}),source_tmdb_id:tmdbId,original_surrogate_tmdb_id:Number(m.tmdb_id)<0?Number(m.tmdb_id):m.raw_tmdb?.original_surrogate_tmdb_id,enriched_at:new Date().toISOString()};
          const patch:any={
            ...
            total_episodes:detail.number_of_episodes||m.total_episodes||null,
            genres,
    ...
    return json({processed:rows.length,ok,fail,resolved_surrogates:resolvedSurrogates,official_refreshed:officialRefreshed,remaining:Math.max(0,ids.length-rows.length),effective_tmdb_ids:true,priority:'current-user'});
