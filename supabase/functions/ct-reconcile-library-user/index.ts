...
const baseTitle=(v:any)=>norm(cleanTitle(v));
const yearOf=(x:any)=>Number(String(x?.release_date||x?.first_air_date||'').slice(0,4))||0;
const aliases=(x:any)=>[x?.title,x?.name,x?.original_title,x?.original_name].map(norm).filter(Boolean);
const positive=(v:any)=>{const n=Number(v||0);return Number.isSafeInteger(n)&&n>0?n:0};
const negative=(v:any)=>{const n=Number(v||0);return Number.isSafeInteger(n)&&n<0?n:0};
const relevant=(x:any)=>Boolean(x?.is_watchlist||x?.is_seen||x?.is_in_progress||x?.is_up_to_date||x?.is_completed||Number(x?.watched_episodes||0)>0||Number(x?.plays||0)>0);
const imported=(m:any)=>Boolean(Number(m?.tmdb_id||0)<0||m?.raw_tmdb?.import_key||m?.raw_tmdb?.bingers_added_at||negative(m?.raw_tmdb?.original_surrogate_tmdb_id));
